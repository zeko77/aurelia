...
function createFileComparer(preferredNames) {
    ...
    return function compareFiles(a, b) {
        const aName = (0, path_1.basename)(a.path);
        const bName = (0, path_1.basename)(b.path);
...
class NPMPackageLoader {
    ...
    async loadEntryPackage(projectDir) {
        ...
        projectDir = (0, path_utils_js_1.normalizePath)(projectDir);
    ...
    async loadPackageCore(dir, issuer) {
        ...
        if (pkg === void 0) {
            ...
            const pkgPath = (0, path_utils_js_1.joinPath)(dir, 'package.json');
    ...
    async resolvePackagePathCore(dep) {
        ...
        if (resolvedPath === void 0) {
            ...
            while (true) {
                resolvedPath = (0, path_utils_js_1.joinPath)(dir, 'node_modules', refName, 'package.json');
                // eslint-disable-next-line no-await-in-loop
                if (await fs.isReadable(resolvedPath)) {
                    break;
                }
                const parent = (0, path_utils_js_1.normalizePath)((0, path_1.dirname)(dir));
                if (parent === dir) {
                    throw new Error(`Unable to resolve npm dependency "${refName}"`);
                }
                dir = parent;
            }
            const realPath = (0, path_utils_js_1.normalizePath)(await fs.getRealPath(resolvedPath));
            if (realPath === resolvedPath) {
                this.logger.debug(`resolved "${refName}" directly to "${(0, path_1.dirname)(realPath)}"`);
            }
            else {
                this.logger.debug(`resolved "${refName}" to "${(0, path_1.dirname)(realPath)}" via symlink "${(0, path_1.dirname)(resolvedPath)}"`);
            }
            resolvedPath = (0, path_utils_js_1.normalizePath)((0, path_1.dirname)(realPath));
    ...
    constructor(loader, files, issuer, pkgJsonFile, dir, pkgJsonFileContent) {
        ...
        if (entryFilePath === void 0) {
            ...
            if (entryFile === void 0) {
                ...
                if (entryFile === void 0) {
                    const withIndexJs = (0, path_utils_js_1.joinPath)(entryFilePath, 'index.js');
