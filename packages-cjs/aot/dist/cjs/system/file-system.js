...
let NodeFileSystem = class NodeFileSystem {
    ...
    readFileSync(path, encoding, cache = false, force = false) {
        ...
        let content = contentCache.get(path);
        if (content === void 0 || force) {
            content = (0, fs_1.readFileSync)(path, encoding);
            if (cache) {
                contentCache.set(path, content);
            }
        }
    ...
    async ensureDir(path) {
        ...
        if (await new Promise(res => { (0, fs_1.exists)(path, res); })) {
            return;
        }
    ...
    writeFileSync(path, content, options) {
        ...
        this.ensureDirSync((0, path_1.dirname)(path));
        (0, fs_1.writeFileSync)(path, content, options);
    }
    async rimraf(path) {
        ...
        try {
            ...
            if (stats.isDirectory()) {
                await Promise.all((await readdir(path)).map(async (x) => this.rimraf((0, path_1.join)(path, x))));
                await rmdir(path);
            }
    ...
    async getRealPath(path) {
        ...
        if (real === void 0) {
            real = (0, path_utils_js_1.normalizePath)(await realpath(path));
    ...
    getRealPathSync(path) {
        ...
        let real = realPathCache.get(path);
        if (real === void 0) {
            real = (0, path_utils_js_1.normalizePath)((0, fs_1.realpathSync)(path));
            realPathCache.set(path, real);
        }
        return real;
    }
    ...
    getChildrenSync(path) {
        ...
        let children = childrenCache.get(path);
        if (children === void 0) {
            children = (0, fs_1.readdirSync)(path).filter(shouldTraverse);
            childrenCache.set(path, children);
        }
        return children;
    }
    async getFiles(root, loadContent = false) {
        ...
        const walk = async (dir, name) => {
            const path = await this.getRealPath((0, path_utils_js_1.joinPath)(dir, name));
            if (seen[path] === void 0) {
                seen[path] = true;
                ...
                if (stats.isFile()) {
                    ...
                    if (ext !== void 0) {
                        const rootlessPath = path.slice((0, path_1.dirname)(root).length);
    ...
    getFilesSync(root, loadContent = false) {
        ...
        const seen = {};
        const walk = (dir, name) => {
            const path = this.getRealPathSync((0, path_utils_js_1.joinPath)(dir, name));
            if (seen[path] === void 0) {
                seen[path] = true;
