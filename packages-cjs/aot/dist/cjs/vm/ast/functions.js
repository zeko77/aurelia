...
class $FunctionExpression {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.FunctionExpression`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        const DirectivePrologue = this.DirectivePrologue = (0, _shared_js_1.GetDirectivePrologue)(node.body.statements);
        ...
        if (!(0, _shared_js_1.hasBit)(modifierFlags, typescript_1.ModifierFlags.Async)) {
    ...
    $EvaluateGenerator(ctx) {
        ...
        const $DefinePropertyOrThrowResult = (0, operations_js_1.$DefinePropertyOrThrow)(ctx, closure, intrinsics.$prototype, new property_descriptor_js_1.$PropertyDescriptor(realm, intrinsics.$prototype, {
            '[[Value]]': prototype,
...
class $FunctionDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.FunctionDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
    ...
    $InstantiateGeneratorFunctionObject(ctx, Scope) {
        ...
        const $DefinePropertyOrThrowResult = (0, operations_js_1.$DefinePropertyOrThrow)(ctx, F, intrinsics.$prototype, new property_descriptor_js_1.$PropertyDescriptor(realm, intrinsics.$prototype, {
            '[[Value]]': prototype,
            '[[Writable]]': intrinsics.true,
            '[[Enumerable]]': intrinsics.false,
            '[[Configurable]]': intrinsics.false,
...
function $FunctionDeclarationInstantiation(ctx, func, argumentsList) {
    ...
    if (argumentsObjectNeeded) {
        ...
        if (strict.isTruthy || !simpleParameterList) {
            ...
            ao = (0, arguments_js_1.$CreateUnmappedArgumentsObject)(ctx, argumentsList);
        }
        // 22. b. Else,
        else {
    ...
    const iteratorRecord = (0, iteration_js_1.$CreateListIteratorRecord)(ctx, argumentsList);
...
class $ArrowFunction {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ArrowFunction`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        if (node.body.kind === typescript_1.SyntaxKind.Block) {
            ...
            this.ContainsUseStrict = false;
            this.$parameters = this.CoveredFormalsList = new $FormalParameterList(node.parameters, this, ctx);
            this.$body = (0, _shared_js_1.$assignmentExpression)(node.body, this, ctx, -1);
        }
        this.functionKind = (0, _shared_js_1.hasBit)(modifierFlags, typescript_1.ModifierFlags.Async) ? 8 /* async */ : 0 /* normal */;
    }
...
class $ConstructorDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ConstructorDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
...
class $ParameterDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ParameterDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        this.logger = logger;
        this.path = path;
        this.modifierFlags = this.combinedModifierFlags = (0, _shared_js_1.modifiersToModifierFlags)(node.modifiers);
