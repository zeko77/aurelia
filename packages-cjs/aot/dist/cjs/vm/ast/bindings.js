...
class $ComputedPropertyName {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ComputedPropertyName`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
...
class $ObjectBindingPattern {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ObjectBindingPattern`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
...
class $ArrayBindingPattern {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ArrayBindingPattern`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
    ...
    InitializeBinding(ctx, value, environment) {
        ...
        const iteratorRecord = (0, iteration_js_1.$GetIterator)(ctx, value);
        ...
        if (iteratorRecord['[[Done]]'].isFalsey) {
            return (0, iteration_js_1.$IteratorClose)(ctx, iteratorRecord, result).enrichWith(ctx, this);
        }
        // 4. Return result.
        return result;
    }
...
class $BindingElement {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.BindingElement`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        ctx = (0, _shared_js_1.clearBit)(ctx, 8 /* IsBindingName */);
        if (node.propertyName === void 0) {
            this.$propertyName = void 0;
            const $name = this.$name = (0, _shared_js_1.$$bindingName)(node.name, this, ctx | 8 /* IsBindingName */, -1);
            ...
            const $propertyName = this.$propertyName = (0, _shared_js_1.$$propertyName)(node.propertyName, this, ctx, -1);
            const $name = this.$name = (0, _shared_js_1.$$bindingName)(node.name, this, ctx | 8 /* IsBindingName */, -1);
            this.BoundNames = $name.BoundNames;
            if (node.initializer === void 0) {
                this.$initializer = void 0;
...
class $SpreadElement {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.SpreadElement`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
    ...
    Evaluate(ctx) {
        ...
        const iteratorRecord = (0, iteration_js_1.$GetIterator)(ctx, spreadObj);
        if (iteratorRecord.isAbrupt) {
            return iteratorRecord.enrichWith(ctx, this);
        }
        // 5. Repeat,
        while (true) {
            ...
            const nextArg = (0, iteration_js_1.$IteratorValue)(ctx, next);
            if (nextArg.isAbrupt) {
    ...
    AccumulateArray(ctx, array, nextIndex) {
        ...
        while (true) {
            ...
            const status = (0, operations_js_1.$CreateDataProperty)(ctx, array, nextIndex.ToUint32(ctx).ToString(ctx), nextValue);
...
class $OmittedExpression {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.OmittedExpression`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
