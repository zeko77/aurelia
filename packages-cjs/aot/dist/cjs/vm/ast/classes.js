...
class $HeritageClause {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.HeritageClause`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
...
class $ExpressionWithTypeArguments {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ExpressionWithTypeArguments`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
...
class $ClassExpression {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ClassExpression`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        const $heritageClauses = this.$heritageClauses = (0, _shared_js_1.$heritageClauseList)(node.heritageClauses, this, ctx);
        const $members = this.$members = (0, _shared_js_1.$$classElementList)(node.members, this, ctx);
        ...
        if ($name === void 0) {
            ...
            if ((0, _shared_js_1.hasAllBits)(modifierFlags, typescript_1.ModifierFlags.ExportDefault)) {
...
class $ClassDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.ClassDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        const intrinsics = realm['[[Intrinsics]]'];
        const modifierFlags = this.modifierFlags = (0, _shared_js_1.modifiersToModifierFlags)(node.modifiers);
        if ((0, _shared_js_1.hasBit)(modifierFlags, typescript_1.ModifierFlags.Export)) {
            ctx |= 4096 /* InExport */;
        }
        this.$decorators = (0, _shared_js_1.$decoratorList)(node.decorators, this, ctx);
        let $name;
        if (node.name === void 0) {
        ...
        if ((0, _shared_js_1.hasBit)(ctx, 4096 /* InExport */)) {
            if ((0, _shared_js_1.hasBit)(this.modifierFlags, typescript_1.ModifierFlags.Default)) {
                if (HasName) {
    ...
    EvaluateClassDefinition(ctx, classBinding, className) {
        ...
        if (constructor instanceof empty_js_1.$Empty) {
            ...
            if (this.ClassHeritage !== void 0) {
                ...
                constructor = this.ConstructorMethod = new functions_js_1.$ConstructorDeclaration((0, typescript_1.createConstructor)(void 0, void 0, [
                    (0, typescript_1.createParameter)(void 0, void 0, (0, typescript_1.createToken)(typescript_1.SyntaxKind.DotDotDotToken), (0, typescript_1.createIdentifier)('args')),
                ], (0, typescript_1.createBlock)([
                    (0, typescript_1.createExpressionStatement)((0, typescript_1.createCall)((0, typescript_1.createSuper)(), void 0, [
                        (0, typescript_1.createSpread)((0, typescript_1.createIdentifier)('args')),
                    ])),
                ])), this, this.ctx, -1);
            }
            // 10. b. Else,
                ...
                constructor = this.ConstructorMethod = new functions_js_1.$ConstructorDeclaration((0, typescript_1.createConstructor)(void 0, void 0, [], (0, typescript_1.createBlock)([])), this, this.ctx, -1);
            }
        }
...
class $PropertyDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.PropertyDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
        ...
        const modifierFlags = this.modifierFlags = (0, _shared_js_1.modifiersToModifierFlags)(node.modifiers);
        this.$decorators = (0, _shared_js_1.$decoratorList)(node.decorators, this, ctx);
        this.$name = (0, _shared_js_1.$$propertyName)(node.name, this, ctx | 512 /* IsMemberName */, -1);
        this.$initializer = (0, _shared_js_1.$assignmentExpression)(node.initializer, this, ctx, -1);
        this.IsStatic = (0, _shared_js_1.hasBit)(modifierFlags, typescript_1.ModifierFlags.Static);
    }
    get $kind() { return typescript_1.SyntaxKind.PropertyDeclaration; }
}
exports.$PropertyDeclaration = $PropertyDeclaration;
class $SemicolonClassElement {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.SemicolonClassElement`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
