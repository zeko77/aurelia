...
class $MethodDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.MethodDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
    ...
    EvaluatePropertyDefinition(ctx, object, enumerable) {
        ...
        return (0, operations_js_1.$DefinePropertyOrThrow)(ctx, object, methodDef['[[Key]]'], desc).enrichWith(ctx, this);
    }
...
class $GetAccessorDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.GetAccessorDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
    ...
    EvaluatePropertyDefinition(ctx, object, enumerable) {
        ...
        return (0, operations_js_1.$DefinePropertyOrThrow)(ctx, object, propKey, desc).enrichWith(ctx, this);
    }
    // http://www.ecma-international.org/ecma-262/#sec-function-definitions-runtime-semantics-evaluatebody
    // 14.1.18 Runtime Semantics: EvaluateBody
    EvaluateBody(ctx, functionObject, argumentsList) {
...
class $SetAccessorDeclaration {
    constructor(node, parent, ctx, idx, mos = parent.mos, realm = parent.realm, depth = parent.depth + 1, logger = parent.logger, path = `${parent.path}${(0, _shared_js_1.$i)(idx)}.SetAccessorDeclaration`) {
        this.node = node;
        this.parent = parent;
        this.ctx = ctx;
        this.idx = idx;
