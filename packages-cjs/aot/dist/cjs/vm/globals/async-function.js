...
class $AsyncFunctionConstructor extends function_js_1.$BuiltinFunction {
    ...
    performSteps(ctx, thisArgument, argumentsList, NewTarget) {
        ...
        return (0, function_js_2.$CreateDynamicFunction)(ctx, this, NewTarget, 8 /* async */, argumentsList);
    }
}
...
function $AsyncFunctionStart(ctx, promiseCapability, asyncFunctionBody) {
    ...
    asyncContext.onResume = function (resumptionValue) {
        ...
        if (result['[[Type]]'] === 1 /* normal */) {
            ...
            (0, operations_js_1.$Call)(asyncContext, promiseCapability['[[Resolve]]'], intrinsics.undefined, new list_js_1.$List(intrinsics.undefined));
        }
        // 3. e. Else if result.[[Type]] is return, then
            ...
            (0, operations_js_1.$Call)(asyncContext, promiseCapability['[[Resolve]]'], intrinsics.undefined, new list_js_1.$List(result));
        }
        // 3. f. Else,
        else {
            // 3. f. i. Assert: result.[[Type]] is throw.
            // 3. f. ii. Perform ! Call(promiseCapability.[[Reject]], undefined, « result.[[Value]] »).
            (0, operations_js_1.$Call)(asyncContext, promiseCapability['[[Reject]]'], intrinsics.undefined, new list_js_1.$List(result)); // TODO: is this cast safe?
        }
        // 3. g. Return.
