...
import { bindingBehavior as r, valueConverter as o, astEvaluator as a, CustomElement as h, attributePattern as l, bindingCommand as c, renderer as u, AttrSyntax as f, BindingMode as d, IAttrMapper as m, IPlatform as g, AttributePattern as p, BindingCommand as b, AppTask as B } from "@aurelia/runtime-html";

import { ValueConverterExpression as T, ISignaler as v, connectable as w, CustomExpression as y, Interpolation as C, IExpressionParser as I, IObserverLocator as P } from "@aurelia/runtime";
...
let W = class TranslationBindingBehavior {
    bind(t, n, i) {
        const s = i.ast.expression;
        if (!(s instanceof T)) {
            const t = new T(s, "t", i.ast.args);
            i.ast.expression = t;
        }
    }
};
...
class TranslationBinding {
    ...
    $bind(t, n) {
        ...
        this.scope = n;
        this.I = this.expr instanceof C;
        this.P = this.expr.evaluate(n, this, this);
        this.M();
        this.parameter?.$bind(t, n);
        this.A(t);
        this.isBound = true;
    }
    $unbind(t) {
    ...
    A(t) {
        ...
        for (const e of n) {
            ...
            for (const e of r) if (this.R(e)) i[e] = n; else {
                const i = h.for(this.target, U);
                const r = i?.viewModel ? this.oL.getAccessor(i.viewModel, e) : this.oL.getAccessor(this.target, e);
                const o = 1 !== this.T.state && (4 & r.type) > 0;
                if (o) s.push(new AccessorUpdateTask(r, n, t, this.target, e)); else r.setValue(n, t, this.target, e);
                this.C.add(r);
            }
        }
        let r = false;
        ...
        if (s.length > 0 || r) this.task = this.taskQueue.queueTask((() => {
            this.task = null;
            for (const t of s) t.run();
            if (r) this.N(i, t);
        }), G);
        e?.cancel();
    }
...
class ParameterBinding {
    ...
    $bind(t, n) {
        ...
        this.value = this.expr.evaluate(n, this, this);
        this.isBound = true;
    }
    $unbind(t) {
        if (!this.isBound) return;
