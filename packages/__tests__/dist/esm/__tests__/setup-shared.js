...
export function $setup(platform) {
    setPlatform(platform);
    BrowserPlatform.set(globalThis, platform);
    const globalStart = platform.performanceNow();
    let firstTestStart = 0;
    let currentRootSuite = (void 0);
    let currentSuite = (void 0);
    let start;
    // eslint-disable-next-line
    beforeEach(function () {
        globalThis['__DEV__'] = typeof process !== 'undefined' ? !!process.env.__DEV__ : false;
        ...
        const nextSuite = this.currentTest.parent;
    ...
    function assertNoWatcher(shouldThrow) {
