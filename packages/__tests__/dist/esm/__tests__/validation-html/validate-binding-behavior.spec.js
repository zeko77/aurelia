...
describe('validation-html/validate-binding-behavior.spec.ts/validate-binding-behavior', function () {
    ...
    let InterceptorBindingBehavior = class InterceptorBindingBehavior extends BindingInterceptor {
    ...
    InterceptorBindingBehavior = __decorate([
    ...
    async function runTest(testFunction, { template, customDefaultTrigger, observeCollection }) {
        ...
        await au
            .register(customDefaultTrigger
            ? ValidationHtmlConfiguration.customize((options) => {
                options.DefaultTrigger = customDefaultTrigger;
            })
            : ValidationHtmlConfiguration, TextBox, EmployeeList, FooBar, ToNumberValueConverter, B64ToPlainTextValueConverter, InterceptorBindingBehavior, VanillaBindingBehavior, Editor, Editor1, Registration.instance(IObserveCollection, observeCollection))
            .app({
            host,
    ...
    $it('works with au-slot - mis-projected part', async function ({ app, host, platform, ctx }) {
        const controller = app.controller;
        const target = host.querySelector('editor #target');
        assertControllerBinding(controller, 'person.name', target, app.controllerRegisterBindingSpy);
        assert.deepStrictEqual(controller.results.filter((r) => !r.valid).map((r) => r.toString()), []);
