...
const { CompositeSegmentExpression, ScopedSegmentExpression, SegmentGroupExpression, SegmentExpression, ComponentExpression, ActionExpression, ViewportExpression, ParameterListExpression, ParameterExpression, } = AST;
const terminal = ['?', '#', '/', '+', '(', ')', '.', '@', '!', '=', ',', '&', '\'', '~', ';'];
describe('route-expression', function () {
    ...
    const paramSpecs = [
        noParams,
        new ParameterListExpression('(1)', [
            new ParameterExpression('1', '0', '1'),
        ]),
        new ParameterListExpression('($1=1)', [
            new ParameterExpression('$1=1', '$1', '1'),
        ]),
        new ParameterListExpression('(1,2)', [
            new ParameterExpression('1', '0', '1'),
            new ParameterExpression('2', '1', '2'),
        ]),
        new ParameterListExpression('($1=1,$2=2)', [
            new ParameterExpression('$1=1', '$1', '1'),
        ...
        for (const viewport of viewportSpecs) {
            for (const action of actionSpecs) {
                for (const scoped of [true, false]) {
                    ...
                    specs[`/${raw}`] = [new RouteExpression(`/${raw}`, true, new SegmentExpression(raw, component, action, viewport, scoped), emptyQuerystring, null, false), url];
                    specs[`/(${raw})`] = [new RouteExpression(`/(${raw})`, true, new SegmentGroupExpression(`(${raw})`, new SegmentExpression(raw, component, action, viewport, scoped)), emptyQuerystring, null, false), `(${url})`];
                    specs[`/${raw}/${raw}`] = [new RouteExpression(`/${raw}/${raw}`, true, new ScopedSegmentExpression(`${raw}/${raw}`, new SegmentExpression(raw, component, action, viewport, scoped), new SegmentExpression(raw, component, action, viewport, scoped)), emptyQuerystring, null, false), `${url}/${url}`];
                    specs[`/${raw}+${raw}`] = [new RouteExpression(`/${raw}+${raw}`, true, new CompositeSegmentExpression(`${raw}+${raw}`, [
                            new SegmentExpression(raw, component, action, viewport, scoped),
                            new SegmentExpression(raw, component, action, viewport, scoped),
                        ], false), emptyQuerystring, null, false), `${url}+${url}`];
                    specs[`/${raw}?foo=bar`] = [new RouteExpression(`/${raw}`, true, new SegmentExpression(raw, component, action, viewport, scoped), Object.freeze(new URLSearchParams('foo=bar')), null, false), `${url}?foo=bar`];
                    specs[`/${raw}?foo=bar&fiz=baz`] = [new RouteExpression(`/${raw}`, true, new SegmentExpression(raw, component, action, viewport, scoped), Object.freeze(new URLSearchParams('foo=bar&fiz=baz')), null, false), `${url}?foo=bar&fiz=baz`];
                    specs[`/${raw}?foo=bar1&fiz=baz&foo=bar2`] = [new RouteExpression(`/${raw}`, true, new SegmentExpression(raw, component, action, viewport, scoped), Object.freeze(new URLSearchParams('foo=bar1&fiz=baz&foo=bar2')), null, false), `${url}?foo=bar1&fiz=baz&foo=bar2`];
                }
            }
        }
    }
