...
describe('I18N', function () {
            ...
            plugins: [
                {
                    type: 'postProcessor',
                    name: 'custom1',
                    process: function (value, _key, _options, _translator) { return value; }
                },
                {
                    type: 'postProcessor',
                    name: 'custom2',
    ...
    ].forEach(({ input, skipTranslationOnMissingKey, output }) => it(`'evaluate' resolves key expression ${input} to ${JSON.stringify(output)}`, async function () {
        const customization = {
            resources: {
                en: { translation }
            },
    ...
    it('setLocale changes the active language of i18next', async function () {
        ...
        eaSpy.methodCalledOnceWith('publish', ["i18n:locale:changed" /* I18N_EA_CHANNEL */, { newLocale: 'de', oldLocale: 'en' }]);
        ...
        assert.deepEqual(args, "aurelia-translation-signal" /* I18N_SIGNAL */);
