...
describe('Scheduler', function () {
    ...
    function queueRecursive(sut, opts, count, cb) {
        function $queue() {
            cb();
            if (--count > 0) {
    ...
    function queueRecursiveAsync(sut, opts, count, cb) {
    ...
    function queueSequential(sut, opts, count, cb) {
        while (count-- > 0) {
            sut.queueTask(cb, opts);
        }
    }
    function queueSequentialAsync(sut, opts, count, cb) {
        while (count-- > 0) {
            sut.queueTask(cb, opts);
        }
    }
        ...
        for (const { sut, name } of prioritySpecs) {
            ...
            describe(`can await ${name}`, function () {
                it('x1, {preempt: false, delay: 0}', function () {
                    const task = sut.queueTask(function () {
                        /* */
                    }, {
                        reusable,
                        preempt: false,
                ...
                for (const iterations of [1, 2, 3]) {
                    describe(`runs until canceled after ${iterations} iterations`, function () {
                        it(`from within the running task`, function (done) {
                            ...
                            const task = sut.queueTask(function () {
                                if (++count === iterations) {
                                    assert.strictEqual(task.status, 1 /* running */, `task.status at count=${count} ${reportTask(task)}`);
                                    task.cancel();
                                    assert.areTaskQueuesEmpty();
                                }
                            }, {
                            ...
                            function callback() {
                                if (++thenCount === iterations) {
                                    assert.strictEqual(task.status, 3 /* canceled */, `task.status at thenCount=${thenCount} ${reportTask(task)}`);
                                    assert.areTaskQueuesEmpty();
                                    done();
                                }
                                else {
                                    assert.strictEqual(task.status, 0 /* pending */, `task.status at thenCount=${thenCount} ${reportTask(task)}`);
                        ...
                        it(`from within a followup task`, function (done) {
                            let count = 0;
                            const task = sut.queueTask(function () {
                                assert.strictEqual(nextTask.status, 0 /* pending */, `nextTask.status in task at count=${count} ${reportTask(nextTask)}`);
                                assert.strictEqual(task.status, 1 /* running */, `task.status in task at count=${count} ${reportTask(task)}`);
                                ++count;
                            }, {
                                persistent: true,
                                reusable,
                            ...
                            function createNextTask() {
                                return sut.queueTask(function () {
                                    assert.strictEqual(nextTask.status, 1 /* running */, `nextTask.status in nextTask at count=${count} ${reportTask(nextTask)}`);
                                    assert.strictEqual(task.status, 0 /* pending */, `task.status in nextTask at count=${count} ${reportTask(task)}`);
                                    if (count === iterations) {
                                        task.cancel();
                                        assert.areTaskQueuesEmpty();
                                    }
                            ...
                            function callback() {
                                if (++thenCount === iterations) {
                                    assert.strictEqual(nextTask.status, 2 /* completed */, `nextTask.status at thenCount=${thenCount} ${reportTask(nextTask)}`);
                                    assert.strictEqual(task.status, 3 /* canceled */, `task.status at thenCount=${thenCount} ${reportTask(task)}`);
                                    assert.areTaskQueuesEmpty();
                                    done();
                                }
                                else {
                                    assert.strictEqual(nextTask.status, 0 /* pending */, `nextTask.status at thenCount=${thenCount} ${reportTask(nextTask)}`);
                                    assert.strictEqual(task.status, 0 /* pending */, `task.status at thenCount=${thenCount} ${reportTask(task)}`);
    ...
    describe('async', function () {
        for (const reusable of [true, false]) {
            ...
            for (const { sut, name } of prioritySpecs) {
                ...
                describe(`can await ${$reusable} ${name}`, function () {
                    ...
                    it('x1, {preempt: false, delay: 0}', async function () {
                        const task = sut.queueTask(async function () {
                            /* */
                        }, {
                            reusable,
                            preempt: false,
                ...
                describe(`can persist ${$reusable} ${name}`, function () {
                    for (const iterations of [1, 2, 3]) {
                        describe(`runs until canceled after ${iterations} iterations`, function () {
                            ...
                            it(`from within a followup task`, async function () {
                                ...
                                let nextTask;
                                function createNextTask() {
                                    return sut.queueTask(async function () {
                                        assert.strictEqual(nextTask.status, 1 /* running */, `nextTask.status in nextTask at count=${count} ${reportTask(nextTask)}`);
                                        assert.strictEqual(task.status, 0 /* pending */, `task.status in nextTask at count=${count} ${reportTask(task)}`);
                                        if (count === iterations) {
                                            task.cancel();
                                ...
                                await task.result;
                                assert.strictEqual(nextTask.status, 0 /* pending */, `nextTask.status after awaiting task.result at thenCount=${thenCount} ${reportTask(nextTask)}`);
                                assert.strictEqual(task.status, 0 /* pending */, `task.status after awaiting task.result at thenCount=${thenCount} ${reportTask(task)}`);
                                await nextTask.result;
                                await callback();
                                await promise;
                            });
                    ...
                    it(`yields after the first iteration with no other tasks, after finishing a persistent task that was canceled from within a followup task`, async function () {
                        const primerTask = sut.queueTask(async function () {
                            assert.strictEqual(primerTask.status, 1 /* running */, `primerTask.status in primerTask ${reportTask(primerTask)}`);
                            assert.strictEqual(primerCancelTask.status, 0 /* pending */, `primerCancelTask.status in primerTask ${reportTask(primerCancelTask)}`);
                        }, {
                            persistent: true,
                            reusable,
                            suspend: true,
                        });
                        const primerCancelTask = sut.queueTask(async function () {
                            assert.strictEqual(primerTask.status, 0 /* pending */, `primerTask.status in primerCancelTask ${reportTask(primerTask)}`);
                            assert.strictEqual(primerCancelTask.status, 1 /* running */, `primerCancelTask.status in primerCancelTask ${reportTask(primerCancelTask)}`);
                            primerTask.cancel();
                            assert.areTaskQueuesEmpty();
                        }, {
                            reusable,
                        ...
                        assert.strictEqual(primerTask.status, 0 /* pending */, `primerTask.status after awaiting primerTask.result ${reportTask(primerTask)}`);
                        assert.strictEqual(primerCancelTask.status, 0 /* pending */, `primerCancelTask.status after awaiting primerTask.result ${reportTask(primerCancelTask)}`);
                        await primerCancelTask.result;
                        assert.strictEqual(primerTask.status, 3 /* canceled */, `primerTask.status after awaiting primerCancelTask.result ${reportTask(primerTask)}`);
                        assert.strictEqual(primerCancelTask.status, 2 /* completed */, `primerCancelTask.status after awaiting primerCancelTask.result ${reportTask(primerCancelTask)}`);
                        assert.areTaskQueuesEmpty();
                        let count = 0;
                        let yieldCount = 0;
                        const persistentTask = sut.queueTask(async function () {
                            assert.strictEqual(persistentTask.status, 1 /* running */, `persistentTask.status in persistentTask ${reportTask(persistentTask)}`);
        ...
        it(`awaits the first task before starting the second`, async function () {
            ...
            async function callback0() {
                states[0] = 1 /* Started */;
                assert.deepStrictEqual(states, [1 /* Started */, 0, 0], `state at the start of callback0`);
                await new Promise(resolve => setTimeout(resolve, 50));
                states[0] = 2 /* Finished */;
                assert.deepStrictEqual(states, [2 /* Finished */, 0, 0], `state at the end of callback0`);
            }
            async function callback1() {
                states[1] = 1 /* Started */;
                assert.deepStrictEqual(states, [2 /* Finished */, 1 /* Started */, 0], `state at the start of callback1`);
                await new Promise(resolve => setTimeout(resolve, 50));
                states[1] = 2 /* Finished */;
                assert.deepStrictEqual(states, [2 /* Finished */, 2 /* Finished */, 0], `state at the end of callback1`);
            }
            async function callback2() {
                states[2] = 1 /* Started */;
                assert.deepStrictEqual(states, [2 /* Finished */, 2 /* Finished */, 1 /* Started */], `state at the start of callback2`);
                await new Promise(resolve => setTimeout(resolve, 50));
                states[2] = 2 /* Finished */;
                assert.deepStrictEqual(states, [2 /* Finished */, 2 /* Finished */, 2 /* Finished */], `state at the end of callback2`);
            }
            const opts = {
                suspend: true,
            };
            ...
            assert.deepStrictEqual(states, [0 /* NotStarted */, 0 /* NotStarted */, 0 /* NotStarted */], `state after queueing 3 tasks`);
            await task0.result;
            ...
            assert.deepStrictEqual(states, [2 /* Finished */, 0 /* NotStarted */, 0 /* NotStarted */], `state after awaiting task0`);
            await task1.result;
            assert.deepStrictEqual(states, [2 /* Finished */, 2 /* Finished */, 0 /* NotStarted */], `state after awaiting task1`);
            await task2.result;
            assert.deepStrictEqual(states, [2 /* Finished */, 2 /* Finished */, 2 /* Finished */], `state after awaiting task2`);
            assert.areTaskQueuesEmpty();
        });
    });
});
