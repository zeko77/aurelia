import { eachCartesianJoin, eachCartesianJoinFactory, createScopeForTest, MockTracingExpression, MockBindingBehavior, MockServiceLocator, createObserverLocator, MockSignaler, MockValueConverter, MockBinding, assert } from '@aurelia/testing';
import { AccessKeyedExpression, AccessMemberExpression, AccessScopeExpression, AccessThisExpression, ArrayLiteralExpression, AssignExpression, BinaryExpression, BindingBehaviorExpression, CallFunctionExpression, CallMemberExpression, CallScopeExpression, ConditionalExpression, Scope, ISignaler, ObjectLiteralExpression, OverrideContext, PrimitiveLiteralExpression, TaggedTemplateExpression, TemplateExpression, UnaryExpression, ValueConverterExpression, DestructuringAssignmentSingleExpression, DestructuringAssignmentRestExpression, DestructuringAssignmentExpression, } from '@aurelia/runtime';
...
function assignDoesNotThrow(inputs) {
    describe('assign() does not throw / is a no-op', function () {
        for (const [text, expr] of inputs) {
            it(`${text}, null`, function () {
                expr.assign(0 /* none */, null, null, null);
            });
        }
    });
}
function throwsOn(expr, method, msg, ...args) {
    let err = null;
    try {
        expr[method](...args);
    }
...
describe('AST', function () {
    ...
    const StringLiteralList = [
        [`''`, PrimitiveLiteralExpression.$empty]
    ];
    const NumberLiteralList = [
    ...
    //   ...KeywordLiteralList
    // ];
    const ArrayLiteralList = [
        [`[]`, $arr]
    ];
    const ObjectLiteralList = [
        [`{}`, $obj]
    ];
    const TemplateLiteralList = [
        [`\`\``, $tpl]
    ];
    ...
    //   ...ObjectLiteralList
    // ];
    const TemplateInterpolationList = [
    ...
    const SimpleTaggedTemplateList = [
        ...AccessScopeList
            .map(([input, expr]) => [`${input}\`\``, new TaggedTemplateExpression([''], [''], expr, [])]),
        ...AccessScopeList
            .map(([input, expr]) => [`${input}\`\${a}\``, new TaggedTemplateExpression(['', ''], ['', ''], expr, [new AccessScopeExpression('a')])])
    ];
    ...
    describe('ValueConverterExpression', function () {
        describe('evaluate() throws when returned converter is nil', function () {
            for (const [text, expr] of SimpleValueConverterList) {
                it(`${text}, undefined`, function () {
                    throwsOn(expr, 'evaluate', `AUR0103:b`, 0 /* none */, dummyScope, dummyLocatorThatReturnsNull, null);
        ...
        describe('assign() throws when returned converter is null', function () {
            for (const [text, expr] of SimpleValueConverterList) {
                it(`${text}, null`, function () {
                    throwsOn(expr, 'assign', `AUR0104:b`, 0 /* none */, dummyScope, dummyLocatorThatReturnsNull, null);
        ...
        describe('connect() throws when returned converter is null', function () {
            for (const [text, expr] of SimpleValueConverterList) {
                it(`${text}, undefined`, function () {
                    throwsOn(expr, 'evaluate', `AUR0103:b`, 0 /* none */, dummyScope, dummyLocator, dummyBindingWithLocatorThatReturnsNull);
    ...
    describe('BindingBehaviorExpression', function () {
        describe('bind() throws when returned behavior is null', function () {
            for (const [text, expr] of SimpleBindingBehaviorList) {
                it(`${text}, undefined`, function () {
                    throwsOn(expr, 'bind', `AUR0101:b`, 0 /* none */, dummyScope, dummyBindingWithLocatorThatReturnsNull);
...
describe('AccessKeyedExpression', function () {
    ...
    it('evaluates member on bindingContext', function () {
        ...
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 'baz', `expression.evaluate(LF.none, scope, null)`);
    });
    ...
    it('assigns member on bindingContext', function () {
        const scope = createScopeForTest({ foo: { bar: 'baz' } });
        expression.assign(0 /* none */, scope, null, 'bang');
    ...
    it('evaluates null/undefined object', function () {
        ...
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), undefined, `expression.evaluate(LF.none, scope, null, null)`);
    ...
    it('does not observes property in keyed object access when key is number', function () {
        ...
        assert.strictEqual(expression2.evaluate(0 /* none */, scope, null, null), 'hello world', `expression2.evaluate(LF.none, scope, null)`);
        const binding = new MockBinding();
        expression2.evaluate(0 /* none */, scope, dummyLocator, binding);
    ...
    it('observes property in keyed array access when key is number', function () {
        ...
        assert.strictEqual(expression3.evaluate(0 /* none */, scope, null, null), 'hello world', `expression3.evaluate(LF.none, scope, null)`);
        const binding = new MockBinding();
        expression3.evaluate(0 /* none */, scope, dummyLocator, binding);
        assert.deepStrictEqual(binding.calls[0], ['observe', scope.bindingContext, 'foo'], 'binding.calls[0]');
        assert.strictEqual(binding.calls.length, 2, 'binding.calls.length');
    });
    describe('does not attempt to observe property when object is primitive', function () {
        ...
        eachCartesianJoin(inputs, (([t1, obj], [t2, key]) => {
            it(`${t1}${t2}`, function () {
                ...
                const binding = new MockBinding();
                sut.evaluate(0 /* none */, scope, dummyLocator, binding);
...
describe('AccessMemberExpression', function () {
    ...
    const inputs = [objects, props];
    const expression = new AccessMemberExpression(new AccessScopeExpression('foo', 0), 'bar');
    eachCartesianJoinFactory.call(this, inputs, (([t1, obj, _isFalsey, canHaveProperty], [t2, prop, value]) => {
        it(`STRICT - ${t1}.${t2}.evaluate() -> connect -> assign`, function () {
            ...
            const actual = sut.evaluate(1 /* isStrictBindingStrategy */, scope, null, null);
            if (canHaveProperty) {
            ...
            if (!(obj instanceof Object)) {
                ...
                sut.assign(0 /* none */, scope, null, 42);
        ...
        it(`${t1}.${t2}.evaluate() -> connect -> assign`, function () {
            const scope = createScopeForTest({ foo: obj });
            const sut = new AccessMemberExpression(new AccessScopeExpression('foo', 0), prop);
            const actual = sut.evaluate(0 /* none */, scope, null, null);
            if (canHaveProperty) {
                if (obj == null) {
    ...
    it('evaluates member on bindingContext', function () {
        const scope = createScopeForTest({ foo: { bar: 'baz' } });
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 'baz', `expression.evaluate(LF.none, scope, null, null)`);
    });
    it('evaluates member on overrideContext', function () {
        const scope = createScopeForTest({});
        scope.overrideContext.foo = { bar: 'baz' };
    ...
    it('returns the assigned value', function () {
        const scope = createScopeForTest({ foo: { bar: 'baz' } });
        assert.strictEqual(expression.assign(0 /* none */, scope, null, 'bang'), 'bang', `expression.assign(LF.none, scope, null, 'bang')`);
    });
    describe('does not attempt to observe property when object is falsey', function () {
        ...
        const inputs2 = [objects2, props2, [true, false]];
    ...
    describe('does not observe if object does not / cannot have the property', function () {
        ...
        const inputs3 = [objects3, props3, [true, false]];
...
describe('AccessScopeExpression', function () {
    ...
    const $parentfoo = new AccessScopeExpression('foo', 1);
    it(`evaluates undefined bindingContext`, function () {
        const scope = Scope.create(undefined, null);
        assert.strictEqual(foo.evaluate(0 /* none */, scope, null, null), '', `foo.evaluate(LF.none, scope, null, null)`);
    });
    it(`evaluates undefined bindingContext STRICT`, function () {
        const scope = Scope.create(undefined, null);
        assert.strictEqual(foo.evaluate(0 /* none */ | 1 /* isStrictBindingStrategy */, scope, null, null), undefined, `foo.evaluate(LF.none | LF.isStrictBindingStrategy, scope, null, null)`);
    });
    it(`assigns undefined bindingContext`, function () {
        const scope = Scope.create(undefined, null);
        foo.assign(0 /* none */, scope, null, 'baz');
        assert.strictEqual(scope.overrideContext.foo, 'baz', `scope.overrideContext.foo`);
    });
    it(`connects undefined bindingContext`, function () {
        const scope = Scope.create(undefined, null);
        const binding = new MockBinding();
        foo.evaluate(0 /* none */, scope, dummyLocator, binding);
    ...
    it(`evaluates defined property on bindingContext`, function () {
        ...
        assert.strictEqual(foo.evaluate(0 /* none */, scope, null, null), 'bar', `foo.evaluate(LF.none, scope, null, null)`);
    });
    ...
    it(`evaluates defined property on first ancestor bindingContext`, function () {
        ...
        assert.strictEqual(foo.evaluate(0 /* none */, scope, null, null), 'bar', `foo.evaluate(LF.none, scope, null, null)`);
        assert.strictEqual($parentfoo.evaluate(0 /* none */, scope, null, null), 'bar', `$parentfoo.evaluate(LF.none, scope, null, null)`);
    });
    it(`evaluates defined property on first ancestor overrideContext`, function () {
        ...
        assert.strictEqual(foo.evaluate(0 /* none */, scope, null, null), 'bar', `foo.evaluate(LF.none, scope, null)`);
        assert.strictEqual($parentfoo.evaluate(0 /* none */, scope, null, null), 'bar', `$parentfoo.evaluate(LF.none, scope, null)`);
    });
    it(`assigns defined property on first ancestor bindingContext`, function () {
        ...
        $parentfoo.assign(0 /* none */, scope, null, 'beep');
    ...
    it(`connects defined property on first ancestor bindingContext`, function () {
        ...
        $parentfoo.evaluate(0 /* none */, scope, dummyLocator, binding);
        assert.strictEqual(binding.calls.length, 1, 'binding.calls.length');
        assert.deepStrictEqual(binding.calls[0], ['observe', scope.parentScope.overrideContext.bindingContext, 'foo'], 'binding.calls[0]');
    });
    ...
    it(`connects undefined property on first ancestor bindingContext`, function () {
        const scope = createScopeForTest({ abc: 'xyz' }, {});
        scope.parentScope.parentScope = Scope.create(undefined, OverrideContext.create({ foo: 'bar' }));
        const binding = new MockBinding();
        $parentfoo.evaluate(0 /* none */, scope, dummyLocator, binding);
        assert.strictEqual(binding.calls.length, 1, 'binding.calls.length');
        assert.deepStrictEqual(binding.calls[0], ['observe', scope.parentScope.overrideContext.bindingContext, 'foo'], 'binding.calls[0]');
...
describe('AccessThisExpression', function () {
    ...
    it('evaluates undefined bindingContext', function () {
        ...
        assert.strictEqual($parent2.evaluate(0 /* none */, scope, null, null), undefined, `$parent2.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
    ...
    it('evaluates null bindingContext', function () {
        ...
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
        scope = { overrideContext: coc(null), parentScope: { overrideContext: coc(null), parentScope: null } };
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), null, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
        ...
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), null, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), null, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
        ...
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), null, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), null, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), null, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
    });
    it('evaluates defined bindingContext', function () {
        ...
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), b, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
        scope = { overrideContext: coc(a), parentScope: { overrideContext: coc(b), parentScope: { overrideContext: coc(c), parentScope: null } } };
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), b, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), c, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), undefined, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
        ...
        assert.strictEqual($parent.evaluate(0 /* none */, scope, null, null), b, `$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent.evaluate(0 /* none */, scope, null, null), c, `$parent$parent.evaluate(LF.none, scope as any, null)`);
        assert.strictEqual($parent$parent$parent.evaluate(0 /* none */, scope, null, null), d, `$parent$parent$parent.evaluate(LF.none, scope as any, null)`);
    });
});
describe('AssignExpression', function () {
    it('can chain assignments', function () {
        ...
        const scope = Scope.create(undefined, null);
        foo.assign(0 /* none */, scope, null, 1);
...
describe('ConditionalExpression', function () {
    it('evaluates the "yes" branch', function () {
        ...
        const sut = new ConditionalExpression(condition, yes, no);
...
describe('BinaryExpression', function () {
    it(`concats strings`, function () {
        ...
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 'ab', `expression.evaluate(LF.none, scope, null, null)`);
        expression = new BinaryExpression('+', new PrimitiveLiteralExpression('a'), $null);
        scope = createScopeForTest({});
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 'a', `expression.evaluate(LF.none, scope, null, null)`);
        expression = new BinaryExpression('+', $null, new PrimitiveLiteralExpression('b'));
        scope = createScopeForTest({});
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 'b', `expression.evaluate(LF.none, scope, null, null)`);
    ...
    it(`adds numbers`, function () {
        ...
        let scope = createScopeForTest({});
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 3, `expression.evaluate(LF.none, scope, null, null)`);
        expression = new BinaryExpression('+', new PrimitiveLiteralExpression(1), $null);
        scope = createScopeForTest({});
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 1, `expression.evaluate(LF.none, scope, null, null)`);
        expression = new BinaryExpression('+', $null, new PrimitiveLiteralExpression(2));
        scope = createScopeForTest({});
        assert.strictEqual(expression.evaluate(0 /* none */, scope, null, null), 2, `expression.evaluate(LF.none, scope, null, null)`);
        expression = new BinaryExpression('+', new PrimitiveLiteralExpression(1), $undefined);
        scope = createScopeForTest({});
        assert.strictEqual(isNaN(expression.evaluate(0 /* none */, scope, null, null)), false, `isNaN(expression.evaluate(LF.none, scope, null, null)`);
        expression = new BinaryExpression('+', $undefined, new PrimitiveLiteralExpression(2));
        scope = createScopeForTest({});
        assert.strictEqual(isNaN(expression.evaluate(0 /* none */, scope, null, null)), false, `isNaN(expression.evaluate(LF.none, scope, null, null)`);
    });
    const flags = 0 /* none */ | 1 /* isStrictBindingStrategy */;
    ...
    it(`adds numbers - STRICT`, function () {
        ...
        expression = new BinaryExpression('+', new PrimitiveLiteralExpression(1), $undefined);
        scope = createScopeForTest({});
        assert.strictEqual(isNaN(expression.evaluate(flags, scope, null, null)), true, `isNaN(expression.evaluate(LF.none | LF.isStrictBindingStrategy,  scope,  null, null)`);
        expression = new BinaryExpression('+', $undefined, new PrimitiveLiteralExpression(2));
        scope = createScopeForTest({});
        assert.strictEqual(isNaN(expression.evaluate(flags, scope, null, null)), true, `isNaN(expression.evaluate(LF.none | LF.isStrictBindingStrategy,  scope,  null, null)`);
    });
    class TestData {
        constructor(expr, expected, scope = createScopeForTest()) {
            this.expr = expr;
    ...
    describe('performs \'in\'', function () {
        ...
        for (const item of getTestData()) {
            it(item.toString(), function () {
                assert.strictEqual(item.expr.evaluate(0 /* none */, item.scope, null, null), item.expected, `expr.evaluate(LF.none, scope, null, null)`);
            });
        }
    });
    describe('performs \'instanceof\'', function () {
        ...
        function* getTestData() {
            for (const scope of [
                ...
                yield new TestData(new BinaryExpression('instanceof', new AccessScopeExpression('foo', 0), new AccessMemberExpression(new AccessScopeExpression('foo', 0), 'constructor')), true, scope);
                yield new TestData(new BinaryExpression('instanceof', new AccessScopeExpression('foo', 0), new AccessMemberExpression(new AccessScopeExpression('bar', 0), 'constructor')), false, scope);
                yield new TestData(new BinaryExpression('instanceof', new AccessScopeExpression('bar', 0), new AccessMemberExpression(new AccessScopeExpression('bar', 0), 'constructor')), true, scope);
                yield new TestData(new BinaryExpression('instanceof', new AccessScopeExpression('bar', 0), new AccessMemberExpression(new AccessScopeExpression('foo', 0), 'constructor')), true, scope);
                yield new TestData(new BinaryExpression('instanceof', new PrimitiveLiteralExpression('foo'), new AccessMemberExpression(new AccessScopeExpression('foo', 0), 'constructor')), false, scope);
            }
...
describe('CallMemberExpression', function () {
    ...
    it(`evaluate handles null/undefined member`, function () {
        const expression = new CallMemberExpression(new AccessScopeExpression('foo', 0), 'bar', []);
        const s1 = createScopeForTest({ foo: {} });
        const s2 = createScopeForTest({ foo: { bar: undefined } });
        const s3 = createScopeForTest({ foo: { bar: null } });
        assert.strictEqual(expression.evaluate(0 /* none */, s1, null, null), undefined, `expression.evaluate(LF.none,  createScopeForTest({ foo: {} }), null, null)`);
        assert.strictEqual(expression.evaluate(0 /* none */, s2, null, null), undefined, `expression.evaluate(LF.none,  createScopeForTest({ foo: { bar: undefined } }), null, null)`);
        assert.strictEqual(expression.evaluate(0 /* none */, s3, null, null), undefined, `expression.evaluate(LF.none,  createScopeForTest({ foo: { bar: null } }), null, null)`);
    });
    it(`evaluate throws when mustEvaluate and member is null or undefined`, function () {
        ...
        assert.throws(() => expression.evaluate(8 /* mustEvaluate */, s1, null, null));
        assert.throws(() => expression.evaluate(8 /* mustEvaluate */, s2, null, null));
        assert.throws(() => expression.evaluate(8 /* mustEvaluate */, s3, null, null));
        assert.throws(() => expression.evaluate(8 /* mustEvaluate */, s4, null, null));
    });
});
describe('CallScopeExpression', function () {
    const foo = new CallScopeExpression('foo', [], 0);
    const hello = new CallScopeExpression('hello', [new AccessScopeExpression('arg', 0)], 0);
    function getScopes(initialScope) {
        return [initialScope];
    }
    it(`evaluates undefined bindingContext`, function () {
        const [scope] = getScopes(Scope.create(undefined, null));
        assert.strictEqual(foo.evaluate(0 /* none */, scope, null, null), undefined, `foo.evaluate(LF.none, scope, null, null)`);
        assert.strictEqual(hello.evaluate(0 /* none */, scope, null, null), undefined, `hello.evaluate(LF.none, scope, null, null)`);
    });
    it(`throws when mustEvaluate and evaluating undefined bindingContext`, function () {
        const [scope] = getScopes(Scope.create(undefined, null));
        assert.throws(() => foo.evaluate(8 /* mustEvaluate */, scope, null, null));
        assert.throws(() => hello.evaluate(8 /* mustEvaluate */, scope, null, null));
    });
    it(`connects undefined bindingContext`, function () {
        const [scope] = getScopes(Scope.create(undefined, null));
        const binding = new MockBinding();
        ...
        hello.evaluate(0 /* none */, scope, dummyLocator, binding);
        assert.strictEqual(binding.calls.length, 1, 'binding.calls.length');
    ...
    it(`evaluates defined property on bindingContext`, function () {
        ...
        assert.strictEqual(foo.evaluate(0 /* none */, scope, null, null), 'bar', `foo.evaluate(LF.none, scope, null, null)`);
        assert.strictEqual(hello.evaluate(0 /* none */, scope, null, null), 'world', `hello.evaluate(LF.none, scope, null, null)`);
    });
    it(`evaluates defined property on overrideContext`, function () {
...
describe('LiteralTemplate', function () {
    class TestData {
        constructor(expr, expected, ctx = {}, hsCtx = null, only = false) {
            this.expr = expr;
            this.expected = expected;
            this.ctx = ctx;
            this.hsCtx = hsCtx;
        ...
        get scope() { return createScopeForTest(this.ctx); }
    }
    function* getTestData() {
        ...
        yield new TestData(new TemplateExpression(['a', 'c', 'e', 'g'], [new PrimitiveLiteralExpression('b'), new PrimitiveLiteralExpression('d'), new PrimitiveLiteralExpression('f')]), 'abcdefg');
        yield new TestData(new TemplateExpression(['a', 'c', 'e'], [new AccessScopeExpression('b', 0), new AccessScopeExpression('d', 0)]), 'a1c2e', { b: 1, d: 2 });
        yield new TestData(new TaggedTemplateExpression([''], [], new AccessScopeExpression('foo', 0)), 'foo', { foo: () => 'foo' });
        yield new TestData(new TaggedTemplateExpression(['foo'], ['bar'], new AccessScopeExpression('baz', 0)), 'foobar', { baz: cooked => `${cooked[0]}${cooked.raw[0]}` });
        yield new TestData(new TaggedTemplateExpression(['1', '2'], [], new AccessScopeExpression('makeString', 0), [new PrimitiveLiteralExpression('foo')]), '1foo2', { makeString: (cooked, foo) => `${cooked[0]}${foo}${cooked[1]}` });
        yield new TestData(new TaggedTemplateExpression(['1', '2'], [], new AccessScopeExpression('makeString', 0), [new AccessScopeExpression('foo', 0)]), '1bar2', { foo: 'bar', makeString: (cooked, foo) => `${cooked[0]}${foo}${cooked[1]}` });
        yield new TestData(new TaggedTemplateExpression(['1', '2', '3'], [], new AccessScopeExpression('makeString', 0), [new AccessScopeExpression('foo', 0), new AccessScopeExpression('bar', 0)]), 'bazqux', { foo: 'baz', bar: 'qux', makeString: (cooked, foo, bar) => `${foo}${bar}` });
        yield new TestData(new TaggedTemplateExpression(['1', '2', '3'], [], new AccessMemberExpression(new AccessScopeExpression('test', 0), 'makeString'), [new AccessScopeExpression('foo', 0), new AccessScopeExpression('bar', 0)]), '1baz2qux3foo', { foo: 'baz', bar: 'qux', test: new Test() });
        yield new TestData(new TaggedTemplateExpression(['1', '2', '3'], [], new AccessKeyedExpression(new AccessScopeExpression('test', 0), new PrimitiveLiteralExpression('makeString')), [new AccessScopeExpression('foo', 0), new AccessScopeExpression('bar', 0)]), '1baz2qux3foo', { foo: 'baz', bar: 'qux', test: new Test() });
    }
    for (const item of getTestData()) {
...
describe('UnaryExpression', function () {
    describe('performs \'typeof\'', function () {
        ...
        for (const { expr, expected } of tests) {
            it(expr.toString(), function () {
                assert.strictEqual(expr.evaluate(0 /* none */, scope, null, null), expected, `expr.evaluate(LF.none, scope, null)`);
            });
        }
    });
    describe('performs \'void\'', function () {
        ...
        for (const { expr } of tests) {
            it(expr.toString(), function () {
                assert.strictEqual(expr.evaluate(0 /* none */, scope, null, null), undefined, `expr.evaluate(LF.none, scope, null)`);
            });
        }
...
describe('BindingBehaviorExpression', function () {
    const flagVariations = [
        () => [`fromBind  `, 2 /* fromBind */],
        () => [`fromUnbind`, 4 /* fromUnbind */]
    ];
    const kindVariations = [
        () => [`0                `, 0],
        () => [`hasBind          `, 2048 /* HasBind */],
        () => [`hasUnbind        `, 4096 /* HasUnbind */],
        () => [`hasBind|hasUnbind`, 2048 /* HasBind */ | 4096 /* HasUnbind */]
    ];
    const inputVariations = [
        ...
        (_$1, [_t2, $kind]) => {
            ...
            expr.$kind = $kind;
            if ($kind & 2048 /* HasBind */) {
                Reflect.defineProperty(expr, 'hasBind', { value: true });
            }
            if ($kind & 4096 /* HasUnbind */) {
                Reflect.defineProperty(expr, 'hasUnbind', { value: true });
            }
            const args = [];
            const sut = new BindingBehaviorExpression(expr, 'mock', args);
            const mock = new MockBindingBehavior();
            const locator = new MockServiceLocator(new Map([['au:resource:binding-behavior:mock', mock]]));
            const observerLocator = createObserverLocator();
            const binding = new PropertyBinding(expr, null, null, null, observerLocator, locator, {});
    ...
    const evaluateVariations = [
        ([_t1, flags], [_t2, $kind], [_t3, scope, sut, mock, _locator, binding, value, _argValues]) => () => {
            ...
            assert.strictEqual(mock.calls.length, 1, `mock.calls.length`);
            const expr = sut.expression;
            const callCount = ($kind & 2048 /* HasBind */) > 0 ? 2 : 1;
    ...
    const connectVariations = [
        ([_t1, flags], [_t2, $kind], [_t3, scope, sut, mock, locator, binding, _value, _argValues]) => () => {
            ...
            assert.strictEqual(mock.calls.length, 1, `mock.calls.length`);
            const expr = sut.expression;
            const callCount = ($kind & 2048 /* HasBind */) > 0 ? 3 : 2;
            assert.strictEqual(expr.calls.length, callCount, `expr.calls.length`);
            assert.strictEqual(expr.calls[callCount - 1].length, 5, `expr.calls[callCount - 1].length`);
            assert.strictEqual(expr.calls[callCount - 1][0], 'evaluate', `expr.calls[callCount - 1][0]`);
            assert.strictEqual(expr.calls[callCount - 1][1], flags, `expr.calls[callCount - 1][1]`);
    ...
    const assignVariations = [
        ([_t1, flags], [_t2, $kind], [_t3, scope, sut, mock, _locator, binding, _value, _argValues]) => () => {
            ...
            assert.strictEqual(mock.calls.length, 1, `mock.calls.length`);
            const expr = sut.expression;
            const callCount = ($kind & 2048 /* HasBind */) > 0 ? 4 : 3;
            assert.strictEqual(expr.calls.length, callCount, `expr.calls.length`);
            assert.strictEqual(expr.calls[callCount - 1].length, 5, `expr.calls[callCount - 1].length`);
    ...
    const $2ndEvaluateVariations = [
        ([_t1, flags], [_t2, $kind], [_t3, scope, sut, mock, _locator, binding, _value, _argValues]) => (newValue) => {
            ...
            assert.strictEqual(mock.calls.length, 1, `mock.calls.length`);
            const expr = sut.expression;
            const callCount = ($kind & 2048 /* HasBind */) > 0 ? 5 : 4;
            assert.strictEqual(expr.calls.length, callCount, `expr.calls.length`);
            assert.strictEqual(expr.calls[callCount - 1].length, 5, `expr.calls[callCount - 1].length`);
            assert.strictEqual(expr.calls[callCount - 1][0], 'evaluate', `expr.calls[callCount - 1][0]`);
            assert.strictEqual(expr.calls[callCount - 1][1], flags, `expr.calls[callCount - 1][1]`);
    ...
    const unbindVariations = [
        ([_t1, flags], [_t2, $kind], [_t3, scope, sut, mock, _locator, binding, _value, _argValues]) => () => {
            ...
            const expr = sut.expression;
            const callCount = ($kind & 2048 /* HasBind */) > 0 ? 6 : 5;
            if ($kind & 4096 /* HasUnbind */) {
                assert.strictEqual(expr.calls.length, callCount, `expr.calls.length`);
    ...
    const inputs = [flagVariations, kindVariations, inputVariations, bindVariations, evaluateVariations, connectVariations, assignVariations, $2ndEvaluateVariations, unbindVariations];
...
describe('ValueConverterExpression', function () {
    ...
    const inputVariations = [
        ...
        (_$1, [_t2, signals, signaler]) => {
            ...
            const expr = new MockTracingExpression(new AccessScopeExpression('foo', 0));
            const args = [];
            const sut = new ValueConverterExpression(expr, 'mock', args);
            const methods = [];
            const mock = new MockValueConverter(methods);
            mock['signals'] = signals;
            const locator = new MockServiceLocator(new Map([['au:resource:value-converter:mock', mock], [ISignaler, signaler]]));
            const observerLocator = createObserverLocator();
            const binding = new PropertyBinding(expr, null, null, null, observerLocator, locator, {});
            const scope = Scope.create({ foo: value }, null);
    ...
    const evaluateWithConnectVariations = [
    ...
    const inputs = [flagVariations, kindVariations, inputVariations, evaluateVariations, evaluateWithConnectVariations, assignVariations, $2ndEvaluateVariations, unbindVariations];
    ...
    function createPrefixedAssertEqual(prefix) {
        return (actual, expected, message) => assert.strictEqual(actual, expected, `[${prefix}]: ${message}`);
    }
});
describe('DestructuringAssignmentExpression', function () {
    describe('DestructuringAssignmentSingleExpression', function () {
        it('{a} = {a:42}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), void 0).assign(0 /* none */, Scope.create(bc), null, { a: 42 });
            assert.strictEqual(bc.a, 42);
        });
        it('{1:a} = {1:"42"}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, '1'), void 0).assign(0 /* none */, Scope.create(bc), null, { 1: '42' });
            assert.strictEqual(bc.a, '42');
        });
        it('{x:a} = {x:"42"}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'x'), void 0).assign(0 /* none */, Scope.create(bc), null, { x: '42' });
            assert.strictEqual(bc.a, '42');
        });
        it('{a=42} = {b:404}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), new PrimitiveLiteralExpression(42)).assign(0 /* none */, Scope.create(bc), null, { b: 404 });
            assert.strictEqual(bc.a, 42);
        });
        it('{1:a=42} = {2:"404"}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, '1'), new PrimitiveLiteralExpression(42)).assign(0 /* none */, Scope.create(bc), null, { 2: "404" });
            assert.strictEqual(bc.a, 42);
        });
        it('{x:a=42} = {b:404}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'x'), new PrimitiveLiteralExpression(42)).assign(0 /* none */, Scope.create(bc), null, { b: 404 });
            assert.strictEqual(bc.a, 42);
        });
        it('{a=404} = {a:42}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), new PrimitiveLiteralExpression(404)).assign(0 /* none */, Scope.create(bc), null, { a: 42 });
            assert.strictEqual(bc.a, 42);
        });
        it('{1:a=404} = {1:"42"}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, '1'), new PrimitiveLiteralExpression(404)).assign(0 /* none */, Scope.create(bc), null, { 1: '42' });
            assert.strictEqual(bc.a, '42');
        });
        it('{x:a=404} = {x:"42"}', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'x'), new PrimitiveLiteralExpression(404)).assign(0 /* none */, Scope.create(bc), null, { x: '42' });
            assert.strictEqual(bc.a, '42');
        });
        it('[a] = [42]', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(0)), void 0).assign(0 /* none */, Scope.create(bc), null, [42]);
            assert.strictEqual(bc.a, 42);
        });
        it('[a=42] = []', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(0)), new PrimitiveLiteralExpression(42)).assign(0 /* none */, Scope.create(bc), null, []);
            assert.strictEqual(bc.a, 42);
        });
        it('[,a=42] = [404]', function () {
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(1)), new PrimitiveLiteralExpression(42)).assign(0 /* none */, Scope.create(bc), null, [404]);
            assert.strictEqual(bc.a, 42);
        });
        it('{a=vm_prop} = {x:404}', function () {
            const ps = Scope.create({ prop: 42 });
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), new AccessScopeExpression('prop', 0)).assign(0 /* none */, Scope.fromParent(ps, bc), null, { x: 404 });
            assert.strictEqual(bc.a, 42);
        });
        it('[,a=vm_prop] = [404]', function () {
            const ps = Scope.create({ prop: 42 });
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(1)), new AccessScopeExpression('prop', 0)).assign(0 /* none */, Scope.fromParent(ps, bc), null, [404]);
            assert.strictEqual(bc.a, 42);
        });
        it('{a=$parent.vm_prop} = {x:404}', function () {
            const ps = Scope.create({ prop: 42 });
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), new AccessScopeExpression('prop', 2)).assign(0 /* none */, Scope.fromParent(Scope.fromParent(ps, Object.create(null)), bc), null, { x: 404 });
            assert.strictEqual(bc.a, 42);
        });
        it('[,a=$parent.vm_prop] = [404]', function () {
            const ps = Scope.create({ prop: 42 });
            const bc = {};
            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(1)), new AccessScopeExpression('prop', 2)).assign(0 /* none */, Scope.fromParent(Scope.fromParent(ps, Object.create(null)), bc), null, [404]);
            assert.strictEqual(bc.a, 42);
        });
    });
    describe('DestructuringAssignmentRestExpression', function () {
        it('{...rest} = {a:1, b:2}', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), []).assign(0 /* none */, Scope.create(bc), null, { a: 1, b: 2 });
            assert.deepStrictEqual(bc, { rest: { a: 1, b: 2 } });
        });
        it('{a, ...rest} = {a:1, b:2}', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), ['a']).assign(0 /* none */, Scope.create(bc), null, { a: 1, b: 2 });
            assert.deepStrictEqual(bc, { rest: { b: 2 } });
        });
        it('{a, b, ...rest} = {a:1, b:2, c:3}', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), ['a', 'b']).assign(0 /* none */, Scope.create(bc), null, { a: 1, b: 2, c: 3 });
            assert.deepStrictEqual(bc, { rest: { c: 3 } });
        });
        it('{a, b, ...rest} = {a:1, b:2}', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), ['a', 'b']).assign(0 /* none */, Scope.create(bc), null, { a: 1, b: 2 });
            assert.deepStrictEqual(bc, { rest: {} });
        });
        it('[...rest] = [1, 2]', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), 0).assign(0 /* none */, Scope.create(bc), null, [1, 2]);
            assert.deepStrictEqual(bc, { rest: [1, 2] });
        });
        it('[,...rest] = [1, 2]', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), 1).assign(0 /* none */, Scope.create(bc), null, [1, 2]);
            assert.deepStrictEqual(bc, { rest: [2] });
        });
        it('[,,...rest] = [1, 2]', function () {
            const bc = {};
            new DestructuringAssignmentRestExpression(new AccessMemberExpression($this, 'rest'), 3).assign(0 /* none */, Scope.create(bc), null, [1, 2]);
            assert.deepStrictEqual(bc, { rest: [] });
        });
    });
    describe('DestructuringAssignmentExpression', function () {
        it('{a} = {a: 1, b:2}', function () {
            const bc = {};
            new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'a'), new AccessMemberExpression($this, 'a'), void 0)
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, { a: 1, b: 2 });
        ...
        it('[a] = [1, 2]', function () {
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, [1, 2]);
            assert.deepStrictEqual(bc, { a: 1 });
        });
        it('[a, b] = [1, 2]', function () {
            const bc = {};
        ...
        it('{prop1, prop2:{prop21}} = {prop1: "foo", prop2: {prop21: 123}}', function () {
            const bc = {};
            new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'prop1'), new AccessMemberExpression($this, 'prop1'), void 0),
                new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, { prop1: 'foo', prop2: { prop21: 123 } });
        ...
        it('{prop1, prop2:{prop21:{prop212:newProp212}, prop22}} = {prop1: "foo", prop2: {prop21: {prop211: 123, prop212: 456}, prop22: "bar" }}', function () {
            ...
            new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                ...
                new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                    new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                        new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'newProp212'), new AccessMemberExpression($this, 'prop212'), void 0),
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, { prop1: 'foo', prop2: { prop21: { prop211: 123, prop212: 456 }, prop22: 'bar' } });
        ...
        it('{prop1,coll:[,{p2:item2p2}]} = {prop1:"foo",coll:[{p1:1,p2:2},{p1:3,p2:4}]}', function () {
            ...
            new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'prop1'), new AccessMemberExpression($this, 'prop1'), void 0),
                new DestructuringAssignmentExpression(90137 /* ArrayDestructuring */, [
                    new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, { prop1: 'foo', coll: [{ p1: 1, p2: 2 }, { p1: 3, p2: 4 }] });
        ...
        it('{prop1,coll:[,{p:[item21]}]} = {prop1:"foo",coll:[{p:[1,2]},{p:[3,4]}]}', function () {
            ...
            new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                ...
                new DestructuringAssignmentExpression(90137 /* ArrayDestructuring */, [
                    new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                        new DestructuringAssignmentExpression(90137 /* ArrayDestructuring */, [
                            new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'item21'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(0)), void 0),
                        ], new AccessMemberExpression($this, 'p'), void 0),
                    ], new AccessKeyedExpression($this, new PrimitiveLiteralExpression(1)), void 0)
                ], new AccessMemberExpression($this, 'coll'), void 0),
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, { prop1: "foo", coll: [{ p: [1, 2] }, { p: [3, 4] }] });
            assert.deepStrictEqual(bc, { prop1: 'foo', item21: 3 });
        });
        it('[k, {prop1, prop2:{prop21}}] = ["key",{prop1: "foo", prop2: {prop21: 123}}]', function () {
            const bc = {};
            new DestructuringAssignmentExpression(90137 /* ArrayDestructuring */, [
                new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'k'), new AccessKeyedExpression($this, new PrimitiveLiteralExpression(0)), void 0),
                new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                    new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'prop1'), new AccessMemberExpression($this, 'prop1'), void 0),
                    new DestructuringAssignmentExpression(106521 /* ObjectDestructuring */, [
                        new DestructuringAssignmentSingleExpression(new AccessMemberExpression($this, 'prop21'), new AccessMemberExpression($this, 'prop21'), void 0),
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, ['key', { prop1: 'foo', prop2: { prop21: 123 } }]);
        ...
        it('[k, [,item2]] = ["key",[1,2]]', function () {
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, ['key', [1, 2]]);
        ...
        it('{a,b:{c}={c:42}} = {a:42}', function () {
            ...
            ], void 0, void 0).assign(0 /* none */, Scope.create(bc), null, { a: 42 });
