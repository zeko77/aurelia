import { ConnectableSwitcher, ProxyObservable } from '@aurelia/runtime';
...
describe('2-runtime/watcher-switcher.spec.ts', function () {
    ...
    it('throws when exiting with non-peek watcher/null', function () {
        ...
        assert.strictEqual(true, ConnectableSwitcher.connecting);
        // eslint-disable-next-line
        assert.throws(() => ConnectableSwitcher.exit({}));
        assert.throws(() => ConnectableSwitcher.exit(null));
        ConnectableSwitcher.exit(dummyWatcher);
    });
    it('watches', function () {
        const logs = [];
        const loggingWatcher = {
            observe(obj, key) {
                logs.push([obj, key]);
            },
            observeCollection(_collection) { },
            subscribeTo(_subscribable) { },
        };
...
//# sourceMappingURL=connectable-switcher.spec.js.map