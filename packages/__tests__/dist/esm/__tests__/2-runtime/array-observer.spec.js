import { ArrayObserver, copyIndexMap, disableArrayObservation, enableArrayObservation, applyMutationsToIndices, synchronizeIndices,
// batch,
...
export class SynchronizingCollectionSubscriber {
    ...
    handleCollectionChange(indexMap, _flags) {
...
describe(`ArrayObserver`, function () {
    ...
    describe('should allow subscribing for immediate notification', function () {
        it('push', function () {
            ...
            sut.subscribe(s);
            arr.push(1);
            assert.deepStrictEqual(s.collectionChanges.pop(), new CollectionChangeSet(0, 0 /* none */, copyIndexMap([-2])));
        });
        it('push 2', function () {
            const s = new SpySubscriber();
            const arr = [];
            ...
            sut.subscribe(s);
            arr.push(1, 2);
            assert.deepStrictEqual(s.collectionChanges.pop(), new CollectionChangeSet(0, 0 /* none */, copyIndexMap([-2, -2])));
        });
    });
    ...
    describe(`observePush`, function () {
        ...
        eachCartesianJoin([
            ...
            it(`size=${padRight(init.length, 2)} itemCount=${padRight(items === null || items === void 0 ? void 0 : items.length, 9)} repeat=${repeat} - behaves as native`, function () {
                const arr = init.slice();
                const expectedArr = init.slice();
                const newItems = items === null || items === void 0 ? void 0 : items.slice();
                sut = new ArrayObserver(arr);
                let expectedResult;
                let actualResult;
                let i = 0;
            ...
            it(`size=${padRight(init.length, 2)} itemCount=${padRight(items === null || items === void 0 ? void 0 : items.length, 9)} repeat=${repeat} - tracks changes`, function () {
                const arr = init.slice();
    ...
    describe(`observeSplice`, function () {
        ...
        eachCartesianJoin([
            ...
            it(`size=${padRight(init.length, 2)} start=${padRight(start, 9)} deleteCount=${padRight(deleteCount, 9)} itemCount=${padRight(items === null || items === void 0 ? void 0 : items.length, 9)} repeat=${repeat} - behaves as native`, function () {
                const arr = init.slice();
                const expectedArr = init.slice();
                const newItems = items === null || items === void 0 ? void 0 : items.slice();
                sut = new ArrayObserver(arr);
                ...
                while (i < repeat) {
                    ...
                    if (items === undefined) {
                        if (deleteCount === undefined) {
                            if (start === undefined) {
                                expectedResult = expectedArr.splice();
                                actualResult = arr.splice();
                            }
                            else {
            ...
            it(`size=${padRight(init.length, 2)} start=${padRight(start, 9)} deleteCount=${padRight(deleteCount, 9)} itemCount=${padRight(items === null || items === void 0 ? void 0 : items.length, 9)} repeat=${repeat} - tracks changes`, function () {
                const arr = init.slice();
                const copy = init.slice();
                const newItems = items === null || items === void 0 ? void 0 : items.slice();
                sut = new ArrayObserver(arr);
...
function incrementItems(items, by) {
    if (items === undefined) {
        return;
    }
    let i = 0;
...
//# sourceMappingURL=array-observer.spec.js.map