import { EventDelegator, EventSubscriber, NodeObserverConfig, } from '@aurelia/runtime-html';
...
function assertHandlerPath(expectedPhases, actualPhases) {
...
function eventPropertiesShallowClone(e) {
    return {
        bubbles: e.bubbles,
...
describe('ListenerTracker', function () {
    function createFixture(eventName, listener, capture, bubbles) {
        ...
        function copyHandler() {
            if (listener == null) {
                ...
                return {
                    handleEvent(e) { return handler(e); },
                };
            }
        }
    ...
    function tearDown({ ctx, sut, el, eventName, listener, capture }) {
...
describe('EventSubscriber', function () {
    function createFixture(listener, eventNames, bubbles) {
        const ctx = TestContext.create();
...
describe('EventDelegator', function () {
    ...
    describe('addEventListener()', function () {
        function createFixture(eventName, listener, bubbles, stopPropagation, returnValue, opts, shadow) {
            const ctx = TestContext.create();
            const childHandlerPath = [];
            const parentHandlerPath = [];
            const childHandler = function (e) {
            ...
            const parentHandler = function (e) {
            ...
            if (shadow !== null) {
                const shadowRoot = wrapper.attachShadow({ mode: shadow });
        ...
        function tearDown({ ctx, wrapper, parentSubscription, childSubscription }) {
        ...
        for (const eventName of ['click']) {
            for (const bubbles of [true, false]) {
                for (const opts of [{ capture: true }, { capture: false }]) {
                            ...
                            for (const shadow of hasShadow ? [null, 'open', 'closed'] : [null]) { // TODO: 'open' should probably work in some way, so we need to try and fix this
                                for (const listenerObj of [null, { handleEvent: null }]) {
                                    it(_ `opts=${opts}, eventName=${eventName}, bubbles=${bubbles}, stopPropagation=${stopPropagation}, returnValue=${returnValue}, shadow=${shadow}, listener=${listenerObj}`, function () {
                                        const { ctx, wrapper, parentEl, childEl, parentSubscription, childSubscription, parentHandlerPath, childHandlerPath, event } = createFixture(eventName, listenerObj, bubbles, stopPropagation, returnValue, opts, shadow);
