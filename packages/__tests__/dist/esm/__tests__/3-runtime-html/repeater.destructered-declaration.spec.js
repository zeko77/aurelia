...
describe('3-runtime-html/repeater.destructured-declaration.spec.ts', function () {
    ...
    }
    {
        class App {
            constructor() {
                this.map = new Map([['a', 1], ['b', 2], ['c', 3]]);
            }
        }
        $it('[k,v] of Map<string, number>', async function (ctx) {
            let expected;
            ...
            await changeAndAssert(ctx, () => { map = ctx.app.map = new Map([['e', 5], ['f', 6]]); }, expected = '<div>e - 5</div><div>f - 6</div>');
            await changeAndAssert(ctx, () => map.set('d', 4), `${expected}<div>d - 4</div>`);
            await changeAndAssert(ctx, () => map.set('d', 44), `${expected}<div>d - 44</div>`);
            await changeAndAssert(ctx, () => map.delete('d'), expected);
    ...
    }
    {
        ...
        $it('change-handling on non-destructured object is operational - [k,p] of Map<string, Person>', async function (ctx) {
    ...
    }
    {
        ...
        $it('change-handling on non-destructured array is operational - [k,ps] of Map<string, Person[]>', async function (ctx) {
            ...
            const map = ctx.app.map;
            // mutation of value
            await changeAndAssert(ctx, () => map.get('81').push(new Person('81843d187e', 11)), '<div>81: 817-1 818-11</div><div>4b: 4bd-84 4b6-73</div><div>6b: 6ba-74</div><div>85: 851-61 851-33 853-81</div>');
            // mutation of map
            await changeAndAssert(ctx, () => map.set('3a', [new Person('3adcb4c20d', 84)]), '<div>81: 817-1 818-11</div><div>4b: 4bd-84 4b6-73</div><div>6b: 6ba-74</div><div>85: 851-61 851-33 853-81</div><div>3a: 3ad-84</div>');
        }, { app: App, template: `<div repeat.for="[k,ps] of map">\${k}:<template repeat.for="p of ps"> \${p.name.slice(0,3)}-\${p.age}</div>` });
    }
});
//# sourceMappingURL=repeater.destructered-declaration.spec.js.map