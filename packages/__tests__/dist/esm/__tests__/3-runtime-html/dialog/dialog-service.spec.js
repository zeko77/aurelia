...
import { IDialogService, IDialogGlobalSettings, DialogConfiguration, DialogDefaultConfiguration, DefaultDialogGlobalSettings, customElement, IDialogDom, IDialogController, INode, DialogController, CustomElement, } from '@aurelia/runtime-html';
...
describe('3-runtime-html/dialog/dialog-service.spec.ts', function () {
    describe('configuration', function () {
        it('throws on empty configuration', async function () {
            let error = void 0;
            try {
                const { startPromise } = createFixture('', class App {
                }, [DialogConfiguration]);
                await startPromise;
            }
            catch (err) {
            ...
            assert.includes(error.message, 'AUR0904');
        });
        it('throws when customize without any implementation', async function () {
            let error = void 0;
            try {
                const { startPromise } = createFixture('', class App {
                }, [DialogConfiguration.customize(noop, [])]);
                await startPromise;
            }
            catch (err) {
                error = err;
            ...
            assert.includes(error.message, 'AUR0012:IDialogGlobalSettings');
        });
    ...
    describe('.open()', function () {
        const testCases = [
                ...
                afterStarted: async (_, dialogService) => {
                    await dialogService.open({
                        component: () => { var _a; return _a = class {
                            ...
                            _a.inject = [IDialogController, IDialogDom, INode],
                            _a; }
                    });
                }
            },
            ...
            ...[null, undefined, true].map(canActivate => ({
                ...
                afterStarted: async function ({ ctx }, dialogService) {
                    ...
                    let TestElement = class TestElement {
                        canActivate() {
            ...
            ...[null, undefined, true].map(canDeactivate => ({
            ...
            })),
            {
                ...
                afterStarted: async (_, dialogService) => {
                    ...
                    assert.strictEqual(result.status, "ok" /* Ok */);
            ...
            },
            {
                ...
                afterStarted: async (_, dialogService) => {
                    ...
                    assert.strictEqual(result.status, "cancel" /* Cancel */);
                }
            },
            {
            ...
            },
            {
                ...
                afterStarted: async ({ ctx }, dialogService) => {
                    ...
                    const overlay = ctx.doc.querySelector('au-dialog-overlay');
                    overlay.click();
                    await Promise.any([
                        dialog.closed,
            ...
            },
            {
                ...
                afterStarted: async ({ ctx }, dialogService) => {
                    const { dialog } = await dialogService.open({ template: 'Hello world', lock: false });
                    assert.strictEqual(ctx.doc.querySelector('au-dialog-container').textContent, 'Hello world');
                    const host = ctx.doc.querySelector('div');
                    host.click();
                    await Promise.any([
                        dialog.closed,
                        new Promise(r => setTimeout(r, 50)),
            ...
            },
            {
                ...
                afterStarted: async ({ ctx }, dialogService) => {
                    ...
                    const buttons = Array.from(ctx.doc.querySelectorAll('[data-dialog-btn]'));
                    buttons[0].click();
                ...
                afterStarted: async (_, dialogService) => {
                    const { dialog } = await dialogService.open({
                        ...
                        component: () => class MyDialog {
                            ...
                            attaching() {
                                var _a, _b;
                                const animation = (_b = (_a = this.host).animate) === null || _b === void 0 ? void 0 : _b.call(_a, [{ transform: 'translateY(0px)' }, { transform: 'translateY(-300px)' }], { duration: 100 });
                                return animation === null || animation === void 0 ? void 0 : animation.finished;
                            }
                            detaching() {
                                var _a, _b;
                                const animation = (_b = (_a = this.host).animate) === null || _b === void 0 ? void 0 : _b.call(_a, [{ transform: 'translateY(-300px)' }, { transform: 'translateY(0)' }], { duration: 100 });
                                return animation === null || animation === void 0 ? void 0 : animation.finished;
                            }
                        },
                    });
        ...
        for (const { title, only, afterStarted, afterTornDown } of testCases) {
            ...
            $it(title, async function () {
                const creationResult = createFixture('', class App {
                }, [DialogDefaultConfiguration]);
                ...
                await tearDown();
                await (afterTornDown === null || afterTornDown === void 0 ? void 0 : afterTornDown(creationResult, dialogService));
...
//# sourceMappingURL=dialog-service.spec.js.map