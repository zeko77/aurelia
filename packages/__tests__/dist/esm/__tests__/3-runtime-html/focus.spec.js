...
describe('focus.spec.ts', function () {
    ...
    describe('basic scenarios', function () {
        ...
        describe('CustomElements -- Initialization only', function () {
            ...
            const shadowModes = ['open', 'closed', null];
            const ceTemplates = [
                '<input />',
                '<textarea></textarea>',
            ...
            eachCartesianJoin([shadowModes, ceTemplates, ceProps], (shadowMode, ceTemplate, ceProp) => {
                ...
                it(`works with ${isFocusable ? 'focusable' : ''} custom element ${ceName}, #shadowRoot: ${shadowMode}`, async function () {
                    ...
                    if (isFocusable) {
                        if (hasShadowRoot) {
                            ...
                            assert.notEqual(activeElement, ceEl, 'Custom element should NOT have focus when it has focusable light dom child');
                        }
                    }
                    assert.equal(component.hasFocus, true, 'It should not have affected component.hasFocus');
                    await dispose();
    ...
    describe('Interactive scenarios', function () {
        ...
        `,
          ...
          <button>Click me</button>
                ...
                assertionFn(ctx, testHost, component, focusable) {
                    const input2 = testHost.querySelector('#input2');
    ...
    function dispatchEventWith(ctx, target, name, bubbles = true) {
