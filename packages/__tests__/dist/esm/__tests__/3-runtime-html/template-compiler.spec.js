...
describe('template-compiler.spec.ts\n  [TemplateCompiler]', function () {
    ...
    describe('compileElement()', function () {
        ...
        describe('with custom element', function () {
            describe('compiles surrogate', function () {
                it('compiles surrogate plain class attribute', function () {
                    ...
                    verifyInstructions(surrogates, [{ toVerify: ['type', 'value'], type: "hf" /* setClassAttribute */, value: 'h-100' }]);
                });
                it('compiles surrogate plain style attribute', function () {
                    ...
                    verifyInstructions(instructions, []);
                    verifyInstructions(surrogates, [{ toVerify: ['type', 'value'], type: "hg" /* setStyleAttribute */, value: 'background: red' }]);
                });
                it('compiles surrogate with binding expression', function () {
                    ...
                    verifyInstructions(surrogates, [{ toVerify: ['type', 'to'], type: "rg" /* propertyBinding */, to: 'class' }], 'surrogate');
                });
                it('compiles surrogate with interpolation expression', function () {
                    ...
                    verifyInstructions(instructions, [], 'normal');
                    verifyInstructions(surrogates, [{ toVerify: ['type', 'to'], type: "rf" /* interpolation */, to: 'class' }], 'surrogate');
                });
            ...
            <el prop1.bind="p" prop2.bind="p" prop3.bind="t" prop3="t"></el>
                ...
                const expectedSiblingInstructions = [
                    { toVerify: ['type', 'res', 'to'], type: "rb" /* hydrateAttribute */, res: CustomAttribute.getDefinition(Prop) },
                    { toVerify: ['type', 'res', 'to'], type: "rb" /* hydrateAttribute */, res: CustomAttribute.getDefinition(Prop) }
                ];
                ...
                const expectedRootInstructions = [
                    { toVerify: ['type', 'res', 'to'], type: "rg" /* propertyBinding */, to: 'prop1' },
                    { toVerify: ['type', 'res', 'to'], type: "rg" /* propertyBinding */, to: 'prop2' }
                ];
            ...
            <el name="name" name2="label"></el>
                ...
                const expectedRootInstructions = [
                    { toVerify: ['type', 'res'], type: "ra" /* hydrateElement */, res: CustomElement.getDefinition(El) }
                ];
                verifyInstructions(rootInstructions, expectedRootInstructions);
                const expectedElInstructions = [
                    { toVerify: ['type', 'to', 'value'], type: "re" /* setProperty */, to: 'name', value: 'name' }
                ];
            ...
            <el background-color="label"></el>
                ...
                const rootInstructions = actual.instructions[0];
                const expectedElInstructions = [
                    { toVerify: ['type', 'value', 'to'], type: "re" /* setProperty */, value: 'label', to: 'backgroundColor' },
                ];
                verifyInstructions(rootInstructions[0].props, expectedElInstructions);
            });
            ...
            describe('with template controller', function () {
                it('compiles', function () {
                    ...
                    assert.strictEqual(template.outerHTML, '<template><au-m class="au"></au-m></template>', `(template as HTMLTemplateElement).outerHTML`);
                ...
                it('moves attrbiutes instructions before the template controller into it', function () {
                    ...
                    verifyInstructions(hydratePropAttrInstruction.props, [
                        { toVerify: ['type', 'to', 'from'],
                            type: "rg" /* propertyBinding */, to: 'value', from: new AccessScopeExpression('p') }
                    ]);
                    verifyInstructions(hydratePropAttrInstruction.def.instructions[0], [
                        { toVerify: ['type', 'to', 'from'],
                            type: "rg" /* propertyBinding */, to: 'name', from: new AccessScopeExpression('name') },
                        { toVerify: ['type', 'to', 'from'],
                            type: "rg" /* propertyBinding */, to: 'title', from: new AccessScopeExpression('title') },
                    ]);
                });
                describe('[as-element]', function () {
                    it('understands [as-element]', function () {
                        ...
                        verifyInstructions(instructions[0], [
                            { toVerify: ['type', 'res'],
                                type: "ra" /* hydrateElement */, res: CustomElement.getDefinition(NotDiv) }
                        ]);
                    });
                        ...
                        it('compiles', function () {
                            ...
                            verifyInstructions(instructions[0], [
                                { toVerify: ['type', 'res', 'to'],
                                    type: "rc" /* hydrateTemplateController */, res: container.find(CustomAttribute, 'if') }
                            ]);
                            const templateControllerInst = instructions[0][0];
                            verifyInstructions(templateControllerInst.props, [
                                { toVerify: ['type', 'to', 'from'],
                                    type: "rg" /* propertyBinding */, to: 'value', from: new AccessScopeExpression('value') }
                            ]);
            ...
            describe('<let/> element', function () {
                ...
                it('ignores custom element resource', function () {
                    ...
                    verifyInstructions(instructions[0], [
                        { toVerify: ['type'], type: "rd" /* hydrateLetElement */ }
                    ]);
                });
                it('compiles with attributes', function () {
                    ...
                    verifyInstructions(instructions[0][0].instructions, [
                        { toVerify: ['type', 'to', 'srcOrExp'],
                            type: "ri" /* letBinding */, to: 'a', from: 'b' },
                        { toVerify: ['type', 'to'],
                            type: "ri" /* letBinding */, to: 'c' }
                    ]);
                });
                describe('[to-binding-context]', function () {
                    ...
                    it('ignores [to-binding-context] order', function () {
                        ...
                        verifyInstructions(instructions, [
                            { toVerify: ['type', 'toBindingContext'], type: "rd" /* hydrateLetElement */, toBindingContext: true }
                        ]);
        ...
        function compileWith(markup, extraResources = []) {
            ...
            const templateDefinition = { template: markup, instructions: [], surrogates: [] };
...
function createTplCtrlAttributeInstruction(attr, value) {
    if (attr === 'repeat.for') {
        return [{
                type: "rk" /* iteratorBinding */,
                from: new ForOfStatement(new BindingIdentifier(value.split(' of ')[0]), new AccessScopeExpression(value.split(' of ')[1])),
                to: 'items'
            }];
    }
    else if (attr.includes('.')) {
        return [{
                type: "rg" /* propertyBinding */,
...
function createTemplateController(ctx, resolveRes, attr, target, value, tagName, finalize, childInstr, childTpl) {
    ...
    if (tagName == null) {
        ...
        const instruction = {
            type: "rc" /* hydrateTemplateController */,
...
function createCustomElement(ctx, tagNameOrDef, finalize, attributes, childInstructions, siblingInstructions, nestedElInstructions, childOutput, childInput, debugMode) {
    ...
    const rawMarkup = `<${def.name} ${attributeMarkup}>${(childInput === null || childInput === void 0 ? void 0 : childInput.template) || ''}</${def.name}>`;
    const input = {
        name: 'unnamed',
        template: finalize ? `<div>${rawMarkup}</div>` : rawMarkup,
        instructions: []
    ...
    const outputAttributeMarkup = debugMode
        ...
        : attributes
            ...
            const canStay = syntax.command === null
                ...
                && exprParser.parse(a[1], 1 /* Interpolation */) === null
                // nor a bindable
            ...
            .map(a => `${a[0]}="${a[1]}"`)
            .join(' ');
    const outputMarkup = ctx.createElementFromMarkup(`<${def.name} ${outputAttributeMarkup}>${(childOutput === null || childOutput === void 0 ? void 0 : childOutput.template.outerHTML) || ''}</${def.name}>`);
...
function createCustomAttribute(ctx, attrNameOrDef, finalize, attributes, childInstructions, siblingInstructions, nestedElInstructions, childOutput, childInput) {
    ...
    const instruction = {
        type: "rb" /* hydrateAttribute */,
        res: attrNameOrDef,
        props: childInstructions
    };
    const attributeMarkup = attributes.map(a => `${a[0]}: ${a[1]};`).join('');
    const rawMarkup = `<div ${resName}="${attributeMarkup}">${(childInput === null || childInput === void 0 ? void 0 : childInput.template) || ''}</div>`;
    const input = {
        name: 'unnamed',
        template: finalize ? `<div>${rawMarkup}</div>` : rawMarkup,
        instructions: []
    ...
    const outputMarkup = ctx.createElementFromMarkup(`<div>${(childOutput === null || childOutput === void 0 ? void 0 : childOutput.template.outerHTML) || ''}</div>`);
    outputMarkup.classList.add('au');
    const output = {
        ...defaultCustomElementDefinitionProperties,
        name: 'unnamed',
...
function createAttributeInstruction(bindableDescription, attributeName, attributeValue, isMulti) {
    ...
    if (!!bindableDescription) {
        if (!!cmd && validCommands.includes(cmd)) {
            const type = "rg" /* propertyBinding */;
            const to = bindableDescription.property;
            const from = parseExpression(attributeValue);
            return { type, to, mode, from };
        }
        else {
            const from = parseExpression(attributeValue, 1 /* Interpolation */);
            if (!!from) {
                const type = "rf" /* interpolation */;
                const to = bindableDescription.property;
                return { type, to, from };
            }
            else {
                const type = "re" /* setProperty */;
                const to = bindableDescription.property;
        ...
        if (!!cmd && validCommands.includes(cmd)) {
            ...
            const from = parseExpression(attributeValue, 1 /* Interpolation */);
            if (!!from) {
                const type2 = "rf" /* interpolation */;
                return { type: type2, to, from };
            }
            else if (isMulti) {
                const type3 = "re" /* setProperty */;
                const to3 = attr;
                const value = attributeValue;
...
describe(`TemplateCompiler - combinations`, function () {
    ...
    describe('plain attributes', function () {
        eachCartesianJoinFactory([
            ...
            [
                (ctx, $1, [, , value]) => [`ref`, value, { type: "rj" /* refBinding */, from: new AccessScopeExpression(value), to: 'element' }],
                (ctx, $1, [attr, to, value]) => [`${attr}.bind`, value, { type: "rg" /* propertyBinding */, from: new AccessScopeExpression(value), to, mode: BindingMode.toView, }],
                (ctx, $1, [attr, to, value]) => [`${attr}.to-view`, value, { type: "rg" /* propertyBinding */, from: new AccessScopeExpression(value), to, mode: BindingMode.toView, }],
                (ctx, $1, [attr, to, value]) => [`${attr}.one-time`, value, { type: "rg" /* propertyBinding */, from: new AccessScopeExpression(value), to, mode: BindingMode.oneTime, }],
                (ctx, $1, [attr, to, value]) => [`${attr}.from-view`, value, { type: "rg" /* propertyBinding */, from: new AccessScopeExpression(value), to, mode: BindingMode.fromView, }],
                (ctx, $1, [attr, to, value]) => [`${attr}.two-way`, value, { type: "rg" /* propertyBinding */, from: new AccessScopeExpression(value), to, mode: BindingMode.twoWay, }],
                (ctx, $1, [attr, to, value]) => [`${attr}.trigger`, value, { type: "hb" /* listenerBinding */, from: new AccessScopeExpression(value), to, strategy: DelegationStrategy.none, preventDefault: true }],
                (ctx, $1, [attr, to, value]) => [`${attr}.delegate`, value, { type: "hb" /* listenerBinding */, from: new AccessScopeExpression(value), to, strategy: DelegationStrategy.bubbling, preventDefault: false }],
                (ctx, $1, [attr, to, value]) => [`${attr}.capture`, value, { type: "hb" /* listenerBinding */, from: new AccessScopeExpression(value), to, strategy: DelegationStrategy.capturing, preventDefault: false }],
                (ctx, $1, [attr, to, value]) => [`${attr}.call`, value, { type: "rh" /* callBinding */, from: new AccessScopeExpression(value), to }]
            ]
        ], (ctx, [el], $2, [n1, v1, i1]) => {
        ...
        for (const debugMode of [true, false]) {
            it(`[Debug: ${debugMode}] [class] attribute + \${interpolation}`, function () {
                ...
                const expected = {
                    ...defaultCustomElementDefinitionProperties,
                    template: ctx.createElementFromMarkup(`<template><span plain data-attr="value" class="${debugMode ? `abc-\${value} ` : ''}au"></span></template>`),
                    instructions: [[
                            {
                                "from": {
                                    "parts": ["abc-", ""],
        ...
        eachCartesianJoinFactory([
            ...
            [
                ...
                (_ctx) => [BindableDefinition.create('asdf', class MyClass {
                    }, { attribute: 'bazBaz', property: 'bazBaz', mode: BindingMode.oneTime }), BindingMode.oneTime, 'bazBaz'],
                (_ctx) => [BindableDefinition.create('asdf', class MyClass {
                    }, { attribute: 'bazBaz', property: 'bazBaz', mode: BindingMode.fromView }), BindingMode.fromView, 'bazBaz'],
                (_ctx) => [BindableDefinition.create('asdf', class MyClass {
                    }, { attribute: 'bazBaz', property: 'bazBaz', mode: BindingMode.twoWay }), BindingMode.twoWay, 'bazBaz'],
                (_ctx) => [BindableDefinition.create('asdf', class MyClass {
                    }, { attribute: 'bazBaz', property: 'bazBaz', mode: BindingMode.default }), BindingMode.default, 'bazBaz']
            ],
            [
                (_ctx) => ['foo', '', class Foo1 {
                    }],
            ...
            [
                (ctx, [, , to], [attr, value]) => [`${attr}`, { type: "re" /* setProperty */, to, value }],
                (ctx, [, mode, to], [attr, value], defaultMode) => [`${attr}.bind`, { type: "rg" /* propertyBinding */, from: value.length > 0 ? new AccessScopeExpression(value) : new PrimitiveLiteralExpression(value), to, mode: (mode && mode !== BindingMode.default) ? mode : (defaultMode || BindingMode.toView) }],
                (ctx, [, , to], [attr, value]) => [`${attr}.to-view`, { type: "rg" /* propertyBinding */, from: value.length > 0 ? new AccessScopeExpression(value) : new PrimitiveLiteralExpression(value), to, mode: BindingMode.toView }],
                (ctx, [, , to], [attr, value]) => [`${attr}.one-time`, { type: "rg" /* propertyBinding */, from: value.length > 0 ? new AccessScopeExpression(value) : new PrimitiveLiteralExpression(value), to, mode: BindingMode.oneTime }],
                (ctx, [, , to], [attr, value]) => [`${attr}.from-view`, { type: "rg" /* propertyBinding */, from: value.length > 0 ? new AccessScopeExpression(value) : new PrimitiveLiteralExpression(value), to, mode: BindingMode.fromView }],
                (ctx, [, , to], [attr, value]) => [`${attr}.two-way`, { type: "rg" /* propertyBinding */, from: value.length > 0 ? new AccessScopeExpression(value) : new PrimitiveLiteralExpression(value), to, mode: BindingMode.twoWay }]
            ]
        ...
        eachCartesianJoinFactory([
            ...
            [
                (ctx, pdName, pdProp) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: kebabCase(pdProp), mode: BindingMode.default }) }),
                (ctx, pdName, pdProp) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: kebabCase(pdProp), mode: BindingMode.oneTime }) }),
                (ctx, pdName, pdProp) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: kebabCase(pdProp), mode: BindingMode.toView }) }),
                (ctx, pdName, pdProp) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: kebabCase(pdProp), mode: BindingMode.fromView }) }),
                (ctx, pdName, pdProp) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: kebabCase(pdProp), mode: BindingMode.twoWay }) })
            ],
            [
                (_ctx) => [``, `''`],
                (_ctx) => [``, `\${a}`],
        ...
        eachCartesianJoinFactory([
            ...
            [
                (ctx, pdName, pdProp, pdAttr) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: pdAttr, mode: BindingMode.default }) }),
                (ctx, pdName, pdProp, pdAttr) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: pdAttr, mode: BindingMode.oneTime }) }),
                (ctx, pdName, pdProp, pdAttr) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: pdAttr, mode: BindingMode.toView }) }),
                (ctx, pdName, pdProp, pdAttr) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: pdAttr, mode: BindingMode.fromView }) }),
                (ctx, pdName, pdProp, pdAttr) => ({ [pdName]: BindableDefinition.create(pdName, class MyClass {
                    }, { property: pdProp, attribute: pdAttr, mode: BindingMode.twoWay }) })
            ],
            [
                (_ctx) => [``, `''`],
                (_ctx) => [``, `\${a}`],
...
describe('TemplateCompiler - local templates', function () {
    ...
    function* getLocalTemplateTestData() {
      ...
      <foo-bar></foo-bar>`, new Map([['foo-bar', new ElementInfo('foo-bar', void 0, false)], ['fiz-baz', new ElementInfo('fiz-baz', void 0, false)]]), new Map([['foo-bar', 1], ['fiz-baz', 1]]), 'static fiz-baz static foo-bar');
        ...
        for (const [bindingMode, props, attributeName] of generateCartesianProduct([
            ...
            const mode = (_a = bindingModeMap.get(bindingMode)) !== null && _a !== void 0 ? _a : BindingMode.toView;
            let bindables = '';
            let templateBody = '';
            let attrExpr = '';
            let renderedContent = '';
   ...
   <foo-bar repeat.for="i of 5" prop.bind="i"></foo-bar>`;
    ...
    <div></div>`;
        ...
        if (__DEV__) {
            const sinks = container.get(DefaultLogger)['warnSinks'];
            ...
            assert.strictEqual(event.severity, 3 /* warn */);
...
describe('TemplateCompiler - au-slot', function () {
    ...
    class TestData {
        constructor(template, customElements, allExpectedProjections, expectedSlotInfos) {
            this.template = template;
    ...
    function* getTestData() {
        ...
        yield new TestData(`<my-element><au-slot au-slot><div au-slot="s1">p1</div><div au-slot="s1">p2</div></au-slot></my-element>`, [$createCustomElement('', 'my-element')], [['my-element', { 'default': '<au-m class="au"></au-m>' }]], []);
    ...
    for (const { customElements, template, expectedSlotInfos, allExpectedProjections } of getTestData()) {
        it(`compiles - ${template}`, function () {
            ...
            for (const expectedSlotInfo of expectedSlotInfos) {
                const actualInstruction = allInstructions.find((i) => i.type === "ra" /* hydrateElement */
            ...
            for (const [elName, projections] of allExpectedProjections) {
                const elementInstruction = allInstructions.find(i => i.type === "ra" /* hydrateElement */
