...
describe('au-slot', function () {
    ...
    function* getTestData() {
        ...
        // tag: mis-projection
        yield new TestData('projection w/o [au-slot] causes mis-projection', `<my-element><div>p</div></my-element>`, [
       ...
       <my-element><div au-slot>p2</div></my-element>`, [
        ...
        ], { 'my-element': [`static default p11root rootp21`, new AuSlotsInfo(['s2', 's1'])] });
        {
            ...
            yield new TestData('supports accessing inner scope with $host', `<my-element> <div au-slot="s2">\${message}</div> <div au-slot="s1">\${$host.message}</div> </my-element>`, [
                CustomElement.define({ name: 'my-element', isStrictBinding: true, template: `<au-slot name="s1">s1</au-slot> <au-slot name="s2">s2</au-slot>` }, MyElement),
        ...
        }
        {
            ...
            yield new TestData('uses inner scope by default if no projection is provided', `<my-element> <div au-slot="s2">\${message}</div> </my-element>`, [
                CustomElement.define({ name: 'my-element', isStrictBinding: true, template: `<au-slot name="s1">\${message}</au-slot> <au-slot name="s2">s2</au-slot>` }, MyElement),
        ...
        `, [
          ...
          <template au-slot>
            \${value}
                ...
                yield new TestData('coping works correctly in conjunction with repeat.for', `<template>
