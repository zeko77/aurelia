...
describe('3-runtime-html/select-value-observer.spec.ts', function () {
    describe('[UNIT]', function () {
        function createFixture(initialValue = '', options = [], multiple = false) {
            ...
            const sut = observerLocator.getObserver(el, 'value');
            sut.setValue(initialValue, 2 /* fromBind */);
        ...
        describe('synchronizeValue()', function () {
            ...
            describe('<select multiple="true" />', function () {
                ...
                it('syncs array & <option/> mutation (from repeat etc...)', async function () {
                    ...
                    currentValue = sut.getValue();
                    assert.deepEqual(currentValue, [
                        { id: 1, name: 'select 1' },
                        { id: 2, name: 'select 2' },
                        'CC',
                    ]);
                ...
                describe('with <optgroup>', function () {
                    it('synchronizes with array', function () {
                        ...
                        sut.syncValue();
                        currentValue = sut.getValue();
                        assert.deepEqual(sut.getValue(), [
                            { id: 1, name: 'select 1' },
                            { id: 2, name: 'select 2' }
                        ]);
                    });
                });
                function createMutiSelectSut(initialValue, optionFactories) {
                    const ctx = TestContext.create();
                    const { observerLocator } = ctx;
                    const el = select(...optionFactories.map(create => create(ctx)))(ctx);
                    const sut = observerLocator.getObserver(el, 'value');
                    sut.setValue(initialValue, 256 /* noFlush */);
                    return { ctx, el, sut };
                }
                function select(...options) {
                    return function (_ctx) {
                        return h('select', { multiple: true }, ...options);
                    };
                }
            });
        });
        ...
        function optgroup(attributes, ...optionFactories) {
            return function (ctx) {
    ...
    it('handles source change', async function () {
      ...
      <button click.trigger="clear()">clear</button>`, class App {
            constructor() {
                this.products = [
...
//# sourceMappingURL=select-value-observer.spec.js.map