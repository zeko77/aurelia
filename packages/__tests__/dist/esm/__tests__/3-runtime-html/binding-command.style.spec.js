import { CustomElement, Aurelia, IEventDelegator, StandardConfiguration } from '@aurelia/runtime-html';
...
function getNormalizedStyle(el, ruleName) {
...
describe('template-compiler.binding-commands.style', function () {
    ...
    const testCases = [
        {
            selector: 'button',
            title: (ruleName, ruleValue, callIndex) => `${callIndex}. ${ruleName}=${ruleValue}`,
            template: (ruleTest) => {
                return `
        ...
        <child repeat.for="i of 5" value.bind="value"></child>`;
            ...
            assert: (au, platform, host, component, [ruleName, ruleValue, ruleDefaultValue, isInvalid, valueOnInvalid], _testCase) => {
                ...
                for (let i = 0, ii = childEls.length; ii > i; ++i) {
                    ...
                    if (hasImportant) {
                        assert.strictEqual(child.style.getPropertyPriority(ruleName), '', `!important[${ruleName}]vm.value="" 1`);
                    }
                }
                component.value = ruleValue;
                platform.domWriteQueue.flush();
                for (let i = 0, ii = childEls.length; ii > i; ++i) {
                    ...
                    if (hasImportant) {
                        assert.strictEqual(child.style.getPropertyPriority(ruleName), 'important', `!important[${ruleName}]component.value=${ruleValue} 2`);
                    }
                }
     ...
     */
    eachCartesianJoin([rulesTests, testCases], ([ruleName, ruleValue, ruleDefaultValue, isInvalid, valueOnInvalid], testCase, callIndex) => {
        it(testCase.title(ruleName, ruleValue, callIndex), async function () {
            var _a;
            const { ctx, au, platform, host, component, tearDown } = createFixture(testCase.template(ruleName), class App {
                constructor() {
            ...
            try {
                ...
                for (let i = 0; ii > i; ++i) {
                    const el = els[i];
                    assert.strictEqual(getNormalizedStyle(el, ruleName), isInvalid ? valueOnInvalid : ruleValueNoPriority, `[${ruleName}]vm.value="${ruleValue}" 1`);
                    if (hasImportant) {
                        assert.strictEqual(el.style.getPropertyPriority(ruleName), 'important', `!important[${ruleName}]vm.value=${ruleValue} 1`);
                    }
                }
                component.value = '';
                platform.domWriteQueue.flush();
                for (let i = 0; ii > i; ++i) {
                    ...
                    if (hasImportant) {
                        assert.strictEqual(el.style.getPropertyPriority(ruleName), '', `!important[${ruleName}]vm.value=${ruleValue} 2`);
                    }
                }
                component.value = ruleValue;
                platform.domWriteQueue.flush();
                for (let i = 0; ii > i; ++i) {
                    ...
                    if (hasImportant) {
                        assert.strictEqual(el.style.getPropertyPriority(ruleName), 'important', `!important[${ruleName}]vm.value=${ruleValue} 3`);
                    }
                }
                component.value = '';
                platform.domWriteQueue.flush();
                for (let i = 0; ii > i; ++i) {
                    ...
                    if (hasImportant) {
                        assert.strictEqual(el.style.getPropertyPriority(ruleName), '', `!important[${ruleName}]vm.value=${ruleValue} 4`);
                    }
                }
                // TODO: for inlined css, there are rules that employs fallback value when incoming value is inappropriate
                //        better test those scenarios
...
//# sourceMappingURL=binding-command.style.spec.js.map