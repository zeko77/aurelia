...
import { MiddlewarePlacement, logMiddleware, localStorageMiddleware, rehydrateFromLocalStorage, executeSteps, LogLevel, STORE } from "@aurelia/store-v1";
import { createStoreWithState, createStoreWithStateAndOptions, createCallCounter } from "./helpers.js";
function mockLocalStorage(patch) {
...
describe("store-v1/middleware.spec.ts", function () {
    ...
    const incrementAction = (currentState) => {
    ...
    it("should allow registering middlewares with additional settings", async function () {
        ...
        const settingsMiddleware = (_, __, settings) => {
            try {
    ...
    it("should allow unregistering middlewares", async function () {
        const store = createStoreWithState(initialState);
        const decreaseBefore = (currentState) => {
            const newState = { ...currentState };
    ...
    it("should not try to delete previously unregistered middlewares", function () {
        ...
        store.middlewares.delete = () => assert.fail("actually tried to delete non existing middleware");
        const decreaseBefore = (currentState) => {
            const newState = { ...currentState };
            newState.counter -= 1000;
            return newState;
    ...
    it("should have a reference to the calling action name and its parameters", async function () {
        ...
        const actionObservedByMiddleware = (state, foo, bar) => {
            return { ...state, counter: foo.length + bar.length };
        };
        const actionAwareMiddleware = (_, __, ___, action) => {
        ...
        await executeSteps(store, false, async () => store.dispatch(actionObservedByMiddleware, "A", "B"), (res) => { assert.equal(res.counter, 2); });
    });
    it("should have a reference all piped actions", async function () {
        ...
        const firstActionObservedByMiddleware = (state, _foo) => state;
        const secondActionObservedByMiddleware = (state, _bar) => state;
        const actionAwareMiddleware = (_, __, ___, action) => {
        ...
        store.registerMiddleware(actionAwareMiddleware, MiddlewarePlacement.After);
        await executeSteps(store, false, async () => store
    ...
    describe("which are applied before action dispatches", function () {
        ...
        it("should support async middlewares", async function () {
            const store = createStoreWithState(initialState);
            const decreaseBefore = async (currentState) => {
                const newState = { ...currentState };
                newState.counter = 0;
        ...
        it("should get additionally the original state, before prev modifications passed in", function (done) {
            const store = createStoreWithState(initialState);
            const decreaseBefore = (currentState, originalState) => {
                const newState = { ...currentState };
                newState.counter = originalState.counter;
                return newState;
            };
            store.registerMiddleware(decreaseBefore, MiddlewarePlacement.Before);
            const resetBefore = (currentState, originalState) => {
            ...
            store.registerAction("IncrementAction", incrementAction);
            store.dispatch(incrementAction).catch(() => { });
            store.state.pipe(skip(1), take(1)).subscribe((state) => {
    ...
    describe("which are applied after the action dispatches", function () {
        ...
        it("should asynchronously change the resulting state", function (done) {
            const store = createStoreWithState(initialState);
            const fixedValueAfter = async (currentState) => {
                const newState = { ...currentState };
                newState.counter = 1000;
                return Promise.resolve(newState);
            };
        ...
        it("should get additionally the original state, before prev modifications passed in", function (done) {
            const store = createStoreWithState(initialState);
            const decreaseAfter = (currentState, originalState) => {
                const newState = { ...currentState };
                newState.counter = originalState.counter;
                return newState;
            };
    ...
    it("should interrupt queue action if middleware returns async false", async function () {
        const store = createStoreWithStateAndOptions(initialState, {});
        const { spyObj } = createCallCounter(store['_state'], "next");
        const syncFalseMiddleware = async () => {
    ...
    it("should handle multiple middlewares", function (done) {
        const store = createStoreWithState(initialState);
        const middlewareFactory = (increaseByX) => (currentState) => {
            const newState = { ...currentState };
    ...
    it("should maintain the order of applying middlewares", function (done) {
        ...
        const middlewareFactory = (value) => (currentState) => {
            const newState = { ...currentState };
            newState.values.push(value);
            return newState;
        };
        new Array(26).fill("")
            .forEach((_, idx) => store.registerMiddleware(middlewareFactory(String.fromCharCode(65 + idx)), MiddlewarePlacement.After));
    ...
    it("should handle middlewares not returning a state", async function () {
        ...
        store.dispatch(incrementAction).catch(() => { });
        await new Promise((resolve) => {
            store.state.pipe(skip(1)).subscribe(() => {
