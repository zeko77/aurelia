...
import { LogLevel, PerformanceMeasurement, UnregisteredActionError, ActionRegistrationError, ReducerNoStateError } from "@aurelia/store-v1";
import { createTestStore, createStoreWithStateAndOptions, createCallCounter } from "./helpers.js";
...
describe("store-v1/store.spec.ts", function () {
    ...
    it("should fail when dispatching unknown actions", function () {
        ...
        return assert.rejects(() => store.dispatch(unregisteredAction), UnregisteredActionError);
    });
    it("should fail when dispatching non actions", function () {
        const { store } = createTestStore();
        return assert.rejects(async () => store.dispatch(undefined), UnregisteredActionError);
    });
    ...
    it("should also accept async false and stop queue", async function () {
        const { store } = createTestStore();
        const { spyObj } = createCallCounter(store._state, "next");
        const fakeAction = async (_) => Promise.resolve(false);
        store.registerAction("FakeAction", fakeAction);
    ...
    it("should unregister previously registered actions", async function () {
        const { store } = createTestStore();
        const fakeAction = (currentState) => currentState;
        store.registerAction("FakeAction", fakeAction);
        await store.dispatch(fakeAction);
    ...
    it("should the previously registered action name as dispatch argument", function (done) {
        const { store } = createTestStore();
        const modifiedState = { foo: "bert" };
        const fakeAction = async (_) => Promise.resolve(modifiedState);
    ...
    it("should dispatch actions one after another", function (done) {
        const { store } = createTestStore();
        const actionA = async (currentState) => Promise.resolve({ foo: `${currentState.foo}A` });
        const actionB = async (currentState) => Promise.resolve({ foo: `${currentState.foo}B` });
        ...
        store.state.pipe(skip(2)).subscribe((state) => {
    ...
    it("should maintain queue of execution in concurrency constraints", function () {
        const { store } = createTestStore();
        createCallCounter(store.dispatchQueue, "push");
        const { spyObj } = createCallCounter(store, "handleQueue");
        const actionA = async (_) => Promise.resolve({ foo: "A" });
        store.registerAction("Action A", actionA);
        store.dispatch(actionA).catch(() => { });
        assert.equal(spyObj.callCounter, 0);
    });
    it("should log info about dispatched action if turned on via options", function () {
        ...
        const store = createStoreWithStateAndOptions(initialState, { logDispatchedActions: true });
        const { spyObj: loggerSpy } = createCallCounter(store.logger, LogLevel.info);
        const actionA = async (_) => Promise.resolve({ foo: "A" });
        store.registerAction("Action A", actionA);
        store.dispatch(actionA).catch(() => { });
    ...
    it("should log info about dispatched action if turned on via options via custom loglevel", function () {
        ...
        const { spyObj: loggerSpy } = createCallCounter(store.logger, LogLevel.debug);
        const actionA = async (_) => Promise.resolve({ foo: "A" });
        store.registerAction("Action A", actionA);
        store.dispatch(actionA).catch(() => { });
        assert.equal(loggerSpy.callCounter, 1);
    ...
    it("should log start-end dispatch duration if turned on via options", async function () {
        ...
        const store = createStoreWithStateAndOptions(initialState, { measurePerformance: PerformanceMeasurement.StartEnd });
        const { spyObj: loggerSpy } = createCallCounter(store.logger, LogLevel.info, false);
        const actionA = async (_) => {
    ...
    it("should log all dispatch durations if turned on via options", async function () {
        ...
        const store = createStoreWithStateAndOptions(initialState, { measurePerformance: PerformanceMeasurement.All });
        const { spyObj: loggerSpy } = createCallCounter(store.logger, LogLevel.info, false);
        const actionA = async (_) => {
            return new Promise((resolve) => {
                setTimeout(() => resolve({ foo: "A" }), 1);
    ...
    it("should reset the state without going through the internal dispatch queue", async function () {
        const { initialState, store } = createTestStore();
        const demoAction = (currentState) => {
            return { ...currentState, foo: "demo" };
        };
        store.registerAction("demoAction", demoAction);
        await store.dispatch(demoAction);
        const { spyObj: internalDispatchSpy } = createCallCounter(store, "internalDispatch");
    ...
    describe("piped dispatch", function () {
        it("should fail when dispatching unknown actions", function () {
            const { store } = createTestStore();
            const unregisteredAction = (currentState, param1) => {
        ...
        it("should fail when at least one action is unknown", function () {
            ...
            const fakeAction = (currentState) => ({ ...currentState });
            store.registerAction("FakeAction", fakeAction);
            const unregisteredAction = (currentState, param1) => ({ ...currentState, foo: param1 });
            const pipedDispatch = store.pipe(fakeAction);
            assert.throws(() => pipedDispatch.pipe(unregisteredAction, "foo"), UnregisteredActionError);
        });
        it("should fail when dispatching non actions", function () {
            const { store } = createTestStore();
            assert.throws(() => store.pipe(undefined), UnregisteredActionError);
        });
        it("should fail when at least one action is no action", function () {
            ...
            store.registerAction("FakeAction", fakeAction);
            const pipedDispatch = store.pipe(fakeAction);
            assert.throws(() => pipedDispatch.pipe(undefined), UnregisteredActionError);
        });
        it("should force reducer to return a new state", function () {
            const { store } = createTestStore();
            const fakeAction = (_) => { };
            store.registerAction("FakeAction", fakeAction);
            return assert.rejects(async () => store.pipe(fakeAction).dispatch(), ReducerNoStateError);
        });
        it("should force all reducers to return a new state", function () {
            const { store } = createTestStore();
            const fakeActionOk = (currentState) => ({ ...currentState });
            const fakeActionNok = (_) => { };
            store.registerAction("FakeActionOk", fakeActionOk);
            store.registerAction("FakeActionNok", fakeActionNok);
            return assert.rejects(async () => store.pipe(fakeActionNok).pipe(fakeActionOk).dispatch(), ReducerNoStateError);
        });
    ...
    describe("internalDispatch", function () {
        it("should throw an error when called with unregistered actions", function () {
            ...
            return assert.rejects(() => store.internalDispatch([{ reducer: unregisteredAction, params: ["foo"] }]), UnregisteredActionError);
        });
        it("should throw an error when one action of multiple actions is unregistered", function () {
            const { store } = createTestStore();
            const registeredAction = (currentState) => currentState;
            const unregisteredAction = (currentState) => currentState;
            store.registerAction("RegisteredAction", registeredAction);
            return assert.rejects(() => store.internalDispatch([
        ...
        it("should throw an error about the first of many unregistered actions", function () {
            const { store } = createTestStore();
            const registeredAction = (currentState) => currentState;
            const firstUnregisteredAction = (currentState) => currentState;
            const secondUnregisteredAction = (currentState) => currentState;
            store.registerAction("RegisteredAction", registeredAction);
            return assert.rejects(store.internalDispatch([
                { reducer: registeredAction, params: [] },
