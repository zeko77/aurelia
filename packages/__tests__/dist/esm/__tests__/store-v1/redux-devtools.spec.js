...
import { Store, DevToolsRemoteDispatchError } from '@aurelia/store-v1';
import { createDI, DevToolsMock, createCallCounter, createTestStore } from './helpers.js';
...
describe("store-v1/redux-devtools.spec.ts", function () {
    ...
    it("should not setup devtools if disabled via options", function () {
        ...
        const store = new Store({ foo: "bar " }, logger, storeWindow, { devToolsOptions: { disable: true } });
    ...
    it("should init devtools if available", function () {
        ...
        const { logger, storeWindow } = createDI({
            __REDUX_DEVTOOLS_EXTENSION__: {
                connect: (devToolsOptions) => new InitTrackerMock(devToolsOptions)
            }
        });
        const store = new Store({ foo: "bar " }, logger, storeWindow);
    });
    it("should use DevToolsOptions if available", function () {
        const { logger, storeWindow } = createDI();
        const options = { serialize: false };
        const store = new Store({ foo: "bar " }, logger, storeWindow, { devToolsOptions: options });
    ...
    describe("dispatching actions", function () {
        it("should react to the ACTION type and execute the intended action", function (done) {
            ...
            const { logger, storeWindow } = createDI();
            const fakeAction = (currentState, newValue) => {
...
//# sourceMappingURL=redux-devtools.spec.js.map