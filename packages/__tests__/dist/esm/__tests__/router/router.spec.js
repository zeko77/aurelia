...
import { IRouter, RouterConfiguration, routes } from '@aurelia/router';
import { CustomElement, customElement, Aurelia, lifecycleHooks } from '@aurelia/runtime-html';
...
describe('Router', function () {
    ...
    async function createFixture(config, App, stateSpy) {
        ...
        const Bar = CustomElement.define({ name: 'bar', template: `<template>Viewport: bar Parameter id: [\${id}] Parameter name: [\${name}] <au-viewport name="bar"></au-viewport></template>` }, (_a = class {
                constructor() {
                    this.id = 'no id';
                    this.name = 'no name';
                    // public binding() {
            ...
            _a.parameters = ['id', 'name'],
            _a));
        const Baz = CustomElement.define({ name: 'baz', template: `<template>Viewport: baz Parameter id: [\${id}] <au-viewport name="baz"></au-viewport></template>` }, (_b = class {
                constructor() {
                    this.id = 'no id';
                }
                load(params) { if (params.id) {
                    this.id = params.id;
                } }
            },
            _b.parameters = ['id'],
        ...
        const au = ctx.wnd['au'] = new Aurelia(container)
            .register(RouterConfiguration.customize(config !== null && config !== void 0 ? config : {}), App)
    ...
    it('handles anchor click with load', async function () {
        ...
        const IdName = CustomElement.define({ name: 'id-name', template: `|id-name| Parameter id: [\${id}] Parameter name: [\${name}]` }, (_a = class {
                constructor() {
                    this.id = 'no id';
                    this.name = 'no name';
                }
        ...
        let App = class App {
            ...
            constructor() { this['IdName'] = IdName; }
        };
        App = __decorate([
    ...
    it('uses overriding reload behavior', async function () {
        ...
        assert.includes(host.textContent, 'Parameter: 123', `host.textContent`);
        assert.includes(host.textContent, 'Entry: 1', `host.textContent`);
        let component = router.getEndpoint('Viewport', 'left').getContent().componentInstance;
        ...
        assert.includes(host.textContent, 'Parameter: 456', `host.textContent`);
        assert.includes(host.textContent, 'Entry: 1', `host.textContent`);
        component = router.getEndpoint('Viewport', 'left').getContent().componentInstance;
    ...
    it.skip('keeps input when stateful', async function () {
        ...
        assert.includes(host.textContent, 'garply', `host.textContent`);
        assert.strictEqual(host.getElementsByTagName('INPUT')[1].value, 'asdf', `(host as any).getElementsByTagName('INPUT')[1].value`);
        await tearDown();
    });
    // }
    ...
    it.skip('keeps children\'s custom element\'s input when navigation history stateful', async function () {
        ...
        const Child = CustomElement.define({ name: 'child', template: '|child|<input> <input type="checkbox" checked.bind="toggle"> <div if.bind="toggle"><input> <au-viewport name="child"></au-viewport></div>', dependencies: [GrandChild] }, class {
            constructor() {
                this.toggle = true;
            }
        });
        ...
        let inputs = host.getElementsByTagName('INPUT');
        ...
        inputs = inputs = host.getElementsByTagName('INPUT');
        for (let i = 0; i < inputs.length; i++) {
            if (typeof values[i] === 'string') {
                assert.strictEqual(inputs[i].value, values[i], `host.getElementsByTagName('INPUT')[${i}].value`);
            }
    ...
    describe('local deps', function () {
        this.timeout(5000);
        async function $setup(dependencies = [], stateSpy) {
            const ctx = TestContext.create();
            const { container, platform } = ctx;
    ...
    describe('can use configuration', function () {
        var _a, _b, _c, _d, _e;
        this.timeout(30000);
        async function $setup(config, dependencies = [], routes = [], stateSpy) {
            var _a;
            const ctx = TestContext.create();
            const { container, platform } = ctx;
            const App = CustomElement.define({
        ...
        const Parent = CustomElement.define({ name: 'parent', template: '!parent!<au-viewport name="parent"></au-viewport>' }, (_a = class {
            },
            _a.routes = [
                { path: 'child-config', instructions: [{ component: 'child', viewport: 'parent' }], title: 'ChildConfig' },
            ],
            _a));
        const Parent2 = CustomElement.define({ name: 'parent2', template: '!parent2!<au-viewport name="parent2"></au-viewport>' }, (_b = class {
            },
            _b.routes = [
            ...
            _b));
        const Child = CustomElement.define({ name: 'child', template: `!child\${param ? ":" + param : ""}!<au-viewport name="child"></au-viewport>` }, (_c = class {
                load(params) {
                    if (params.id !== void 0) {
                        this.param = params.id;
                    }
            ...
            _c));
        const Child2 = CustomElement.define({ name: 'child2', template: `!child2\${param ? ":" + param : ""}!<au-viewport name="child2"></au-viewport>` }, (_d = class {
                load(params) {
                    if (params.id !== void 0) {
                        this.param = params.id;
                    }
            ...
            _d.parameters = ['id'],
            _d.title = (vm) => vm.param !== void 0 ? vm.param : 'Child2',
            _d));
        const Grandchild = CustomElement.define({ name: 'grandchild', template: '!grandchild!' });
        const Grandchild2 = CustomElement.define({ name: 'grandchild2', template: '!grandchild2!' }, (_e = class {
            },
            _e.title = 'TheGrandchild2',
            _e));
        const tests = [
        ...
        const appRoutes = [
            ...
            { path: 'parent-config/:id', instructions: [{ component: 'parent', viewport: 'default', children: [{ component: 'child', viewport: 'parent' }] }], title: (instruction) => { var _a; return `ParentConfig${(_a = instruction.parameters.get(instruction.scope.router, 'id')) !== null && _a !== void 0 ? _a : ':id'}Config`; } },
    ...
    describe('can use title configuration', function () {
        ...
        const Parent = CustomElement.define({ name: 'my-parent', template: '!my-parent!<au-viewport name="parent"></au-viewport>' }, (_a = class {
            },
            _a.title = 'TheParent',
            _a.routes = [
            ...
            _a));
        const Parent2 = CustomElement.define({ name: 'my-parent2', template: '!my-parent2!<au-viewport name="parent2"></au-viewport>' }, (_b = class {
            },
            _b.routes = [
                { path: 'child-config', instructions: [{ component: 'my-child', viewport: 'parent2' }], title: 'TheChildConfig' },
            ],
            _b));
        const Child = CustomElement.define({ name: 'my-child', template: `!my-child\${param ? ":" + param : ""}!<au-viewport name="child"></au-viewport>` }, (_c = class {
                load(params) {
                    if (params.id !== void 0) {
                        this.param = params.id;
                    }
            ...
            _c.title = (vm) => `TheChild${vm.param !== void 0 ? `(${vm.param})` : ''}`,
            _c));
        const Child2 = CustomElement.define({ name: 'my-child2', template: `!my-child2\${param ? ":" + param : ""}!<au-viewport name="child2"></au-viewport>` }, (_d = class {
                load(params) {
                    if (params.id !== void 0) {
                        this.param = params.id;
                    }
            ...
            _d.parameters = ['id'],
            _d.title = (vm) => `TheChild2${vm.param !== void 0 ? `(${vm.param})` : ''}`,
            _d));
        const titleConfigs = [
        ...
        const appRoutes = [
            ...
            { path: 'parent-config/:id', instructions: [{ component: 'my-parent', viewport: 'default', children: [{ component: 'my-child', viewport: 'parent' }] }], title: (instruction) => { var _a; return `TheParentConfig(${(_a = instruction.parameters.get(instruction.scope.router, 'id')) !== null && _a !== void 0 ? _a : ':id'})Config`; } },
    ...
    describe('can use lifecycleHooks', function () {
        ...
        let Hooks = class Hooks {
            ...
            canLoad(vm) { calledHooks.push(`${this.name}:${vm.name}:canLoad`); return true; }
            load(vm) { calledHooks.push(`${this.name}:${vm.name}:load`); }
            canUnload(vm) { calledHooks.push(`${this.name}:${vm.name}:canUnload`); return true; }
            unload(vm) { calledHooks.push(`${this.name}:${vm.name}:unload`); }
        };
        Hooks = __decorate([
            lifecycleHooks()
        ], Hooks);
        const One = CustomElement.define({ name: 'my-one', template: '!my-one!', dependencies: [Hooks] }, class {
            constructor() {
                this.name = 'my-one';
            }
        });
