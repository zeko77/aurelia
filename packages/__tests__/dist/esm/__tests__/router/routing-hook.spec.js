...
describe('RoutingHook', function () {
    this.timeout(5000);
    async function createFixture(config, App, dependencies = [], stateSpy) {
    ...
    it('uses a hook', async function () {
        ...
        RouterConfiguration.for(router).addHook((url, navigationInstruction) => Promise.resolve(`hooked:${url}`), { type: 'transformFromUrl' });
    ...
    it('uses consequtive hooks', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((url, navigationInstruction) => Promise.resolve(`hooked:${url}`), { type: 'transformFromUrl' });
        RouterConfiguration.for(router).addHook((url, navigationInstruction) => Promise.resolve(`hooked2:${url}`), { type: 'transformFromUrl' });
        const hooked = await RoutingHook.invokeTransformFromUrl('testing', navigationInstruction);
    ...
    it('uses a TransformFromUrl hook returning viewport instructions', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((url, navigationInstruction) => Promise.resolve([RoutingInstruction.create(`hooked-${url}`)]), { type: 'transformFromUrl' });
        const hooked = await RoutingHook.invokeTransformFromUrl('testing', navigationInstruction);
    ...
    it('uses a TransformToUrl hook getting viewport instructions returning viewport instructions', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((routingInstructions, navigationInstruction) => Promise.resolve([RoutingInstruction.create(`hooked-${routingInstructions[0].component.name}`)]), { type: 'transformToUrl' });
        const hooked = await RoutingHook.invokeTransformToUrl([RoutingInstruction.create('testing')], navigationInstruction);
    ...
    it('uses a TransformToUrl hook getting viewport instructions returning string', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((routingInstructions, navigationInstruction) => Promise.resolve(`hooked-${routingInstructions[0].component.name}`), { type: 'transformToUrl' });
        const hooked = await RoutingHook.invokeTransformToUrl([RoutingInstruction.create('testing')], navigationInstruction);
        assert.strictEqual(hooked, 'hooked-testing', `hooked`);
        await tearDown();
    });
    it('uses a TransformToUrl hook getting string returning viewport instructions', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((url, navigationInstruction) => Promise.resolve([RoutingInstruction.create(`hooked-${url}`)]), { type: 'transformToUrl' });
        const hooked = await RoutingHook.invokeTransformToUrl('testing', navigationInstruction);
        assert.strictEqual(hooked[0].component.name, 'hooked-testing', `hooked`);
        await tearDown();
    });
    it('uses a TransformToUrl hook getting string returning string', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((url, navigationInstruction) => Promise.resolve(`hooked-${url}`), { type: 'transformToUrl' });
        const hooked = await RoutingHook.invokeTransformToUrl('testing', navigationInstruction);
        assert.strictEqual(hooked, 'hooked-testing', `hooked`);
        await tearDown();
    });
    it('uses a TransformToUrl hook with alternating types', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        const hook = (input, navigationInstruction) => Promise.resolve(typeof input === 'string' ? [RoutingInstruction.create(`hooked-${input}`)] : `hooked-${input[0].component.name}`);
        const str = 'testing';
        RouterConfiguration.for(router).addHook(hook, { type: 'transformToUrl' });
        let hooked = await RoutingHook.invokeTransformToUrl(str, navigationInstruction);
        assert.strictEqual(hooked[0].component.name, `hooked-${str}`, `hooked`);
        RouterConfiguration.for(router).addHook(hook, { type: 'transformToUrl' });
        hooked = await RoutingHook.invokeTransformToUrl(str, navigationInstruction);
    ...
    it('uses a BeforeNavigation hook returning viewport instructions', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((routingInstructions, navigationInstruction) => Promise.resolve([RoutingInstruction.create(`hooked-${routingInstructions[0].component.name}`)]), { type: 'beforeNavigation' });
        const hooked = await RoutingHook.invokeBeforeNavigation([RoutingInstruction.create('testing')], navigationInstruction);
        assert.strictEqual(hooked[0].component.name, 'hooked-testing', `hooked`);
        await tearDown();
    });
    it('uses a BeforeNavigation hook returning true', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((routingInstructions, navigationInstruction) => Promise.resolve(true), { type: 'beforeNavigation' });
        const hooked = await RoutingHook.invokeBeforeNavigation([RoutingInstruction.create('testing')], navigationInstruction);
    ...
    it('uses a BeforeNavigation hook returning false', async function () {
        const { router, tearDown, navigationInstruction } = await createFixture();
        RouterConfiguration.for(router).addHook((routingInstructions, navigationInstruction) => Promise.resolve(false), { type: 'beforeNavigation' });
        const hooked = await RoutingHook.invokeBeforeNavigation([RoutingInstruction.create('testing')], navigationInstruction);
    ...
    it('sets a TransformToUrl hook with alternating types during initialization', async function () {
        const hookFunction = (input, navigationInstruction) => input.length > 0
            ? typeof input === 'string' ? [RoutingInstruction.create(`hooked-${input}`)] : `hooked-${input[0].component.name}`
            : input;
        ...
        const str = 'testing';
        const hooked = await RoutingHook.invokeTransformToUrl(str, navigationInstruction);
        assert.strictEqual(hooked[0].component.name, `hooked-hooked-hooked-${str}`, `hooked-hooked-hooked`);
        await tearDown();
    });
    it('sets a TransformToUrl hook with alternating types through api', async function () {
        ...
        const hookFunction = (input, navigationInstruction) => Promise.resolve(input.length > 0
            ? typeof input === 'string' ? [RoutingInstruction.create(`hooked-${input}`)] : `hooked-${input[0].component.name}`
            : input);
    ...
    it('can prevent navigation', async function () {
        ...
        RouterConfiguration.for(router).addHook((instructions, navigation) => Promise.resolve(false), { type: 'beforeNavigation', include: ['two'] });
        await $load('one', router, platform);
        assert.strictEqual(host.textContent, `!one!`, `one`);
        await $load('two', router, platform);
        assert.strictEqual(host.textContent, `!one!`, `one`);
    ...
    it('can redirect navigation', async function () {
        ...
        await $load('two', router, platform);
        assert.strictEqual(host.textContent, `!two!`, `two`);
        RouterConfiguration.for(router).addHook((instructions, navigation) => Promise.resolve([RoutingInstruction.create('three', instructions[0].viewport.instance)]), { type: 'beforeNavigation', include: ['two'] });
        await $load('one', router, platform);
        assert.strictEqual(host.textContent, `!one!`, `one`);
        await $load('two', router, platform);
    ...
    it('can transform from url to string', async function () {
        ...
        await $load('two', router, platform);
        assert.strictEqual(host.textContent, `!two!`, `two`);
        RouterConfiguration.for(router).addHook((url, navigation) => Promise.resolve(url === 'two' ? 'three' : url), { type: 'transformFromUrl' });
        await $load('one', router, platform);
        assert.strictEqual(host.textContent, `!one!`, `one`);
        await $load('two', router, platform);
        assert.strictEqual(host.textContent, `!three!`, `three`);
    ...
    it('can transform from url to viewport instructions', async function () {
        ...
        await $load('two', router, platform);
        assert.strictEqual(host.textContent, `!two!`, `two`);
        RouterConfiguration.for(router).addHook((url, navigation) => Promise.resolve(url === 'two' ? [RoutingInstruction.create('three')] : url), { type: 'transformFromUrl' });
        await $load('one', router, platform);
        assert.strictEqual(host.textContent, `!one!`, `one`);
        await $load('two', router, platform);
        assert.strictEqual(host.textContent, `!three!`, `three`);
    ...
    it('can transform from viewport instructions to url', async function () {
        ...
        RouterConfiguration.for(router).addHook((state, navigation) => Promise.resolve(typeof state === 'string'
            ? state === 'two' ? 'hooked-two' : state
            : state[0].component.name === 'two' ? 'hooked-two' : state), { type: 'transformToUrl' });
        await $load('one', router, platform);
        assert.strictEqual(host.textContent, `!one!`, `one`);
        assert.strictEqual(locationPath, `#/one`, `locationPath one`);
        await $load('two', router, platform);
...
//# sourceMappingURL=routing-hook.spec.js.map