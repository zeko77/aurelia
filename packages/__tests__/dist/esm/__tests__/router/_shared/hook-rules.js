...
export function verifyRules(invocations, swapStrategy, phase, root, from, to) {
    const { viewports, topViewport, fromViewports, toViewports, underlyingFrom, underlyingTo, } = getViewports(root, from, to);
...
function verifyDeactivateHooksOrder(invocations, phase, viewports) {
    for (let i = viewports.length - 1; i > 0; i--) {
        if (!verifyHookOrder(invocations, `${phase}:${viewports[i].name}.${viewports[i].from.name}.detaching.enter`, `${phase}:${viewports[i - 1].name}.${viewports[i - 1].from.name}.detaching.enter`)) {
            console.log('ERROR in deactivate hooks order, detaching', phase, invocations);
            return false;
        }
    }
    for (let i = viewports.length - 1; i > 0; i--) {
        if (!verifyHookOrder(invocations, `${phase}:${viewports[i].name}.${viewports[i].from.name}.unbinding.enter`, `${phase}:${viewports[i - 1].name}.${viewports[i - 1].from.name}.unbinding.enter`)) {
            console.log('ERROR in deactivate hooks order, unbinding', phase, invocations);
            return false;
        }
    }
    for (let i = 0, ii = viewports.length - 1; i < ii; i++) {
        if (!verifyHookOrder(invocations, `${phase}:${viewports[i].name}.${viewports[i].from.name}.dispose.enter`, `${phase}:${viewports[i + 1].name}.${viewports[i + 1].from.name}.dispose.enter`)) {
...
function verifyHookOrder(invocations, ...verify) {
...
function getViewportHooks(swapStrategy, phase, viewport, type = '') {
    const hooks = [];
    switch (type) {
        case 'activate':
...
//   return true;
// }
function getViewports(root, from, to) {
    var _a, _b;
    ...
    let toPath = fromPath;
    const topViewport = new Viewport(fromPath, ((_a = froms === null || froms === void 0 ? void 0 : froms[0]) !== null && _a !== void 0 ? _a : ''), ((_b = tos === null || tos === void 0 ? void 0 : tos[0]) !== null && _b !== void 0 ? _b : ''), true);
    const viewports = [topViewport];
    // The "old" viewports being cleared
    const fromViewports = !topViewport.from.isEmpty ? [topViewport] : [];
    // The new viewports loading new content
    const toViewports = !topViewport.to.isEmpty ? [topViewport] : [];
...
// }
//# sourceMappingURL=hook-rules.js.map