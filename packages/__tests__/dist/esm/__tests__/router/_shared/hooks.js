...
export const routingHooks = ['canUnload', 'canLoad', 'unload', 'load'];
export const addHooks = ['binding', 'bound', 'attaching', 'attached'];
export const removeHooks = ['detaching', 'unbinding', 'dispose'];
export function* getStartHooks(root) {
    yield `start.${root}.binding`;
    yield `start.${root}.bound`;
    yield `start.${root}.attaching`;
    yield `start.${root}.attached`;
}
export function* getStopHooks(root, p, c = '', c3 = '', c4 = '') {
    yield `stop.${root}.detaching`;
    if (p) {
        yield* prepend('stop', p, 'unload', 'detaching');
    }
    yield `stop.${root}.unbinding`;
    if (c) {
        yield* prepend('stop', c, 'unload', 'detaching');
    }
    if (c3) {
        yield* prepend('stop', c3, 'unload', 'detaching');
    }
    if (c4) {
        yield* prepend('stop', c4, 'unload', 'detaching');
    }
    if (p) {
        yield* prepend('stop', p, 'unbinding');
    }
    if (c) {
        yield* prepend('stop', c, 'unbinding');
    }
    if (c3) {
        yield* prepend('stop', c3, 'unbinding');
    }
    if (c4) {
        yield* prepend('stop', c4, 'unbinding');
    }
}
function removeIndex(transitions, index) {
...
function getViewports(transitions, forceParallel) {
    var _a, _b;
    const viewports = [];
    let topViewport;
    ...
    const topFrom = (_a = topViewports.filter(viewport => !viewport.from.isEmpty)) === null || _a === void 0 ? void 0 : _a[0];
    const topTo = (_b = topViewports.filter(viewport => !viewport.to.isEmpty)) === null || _b === void 0 ? void 0 : _b[0];
    return {
        viewports,
        topViewports,
        addViewports,
...
function getTypedViewports(viewports) {
    var _a, _b;
    const topViewports = viewports.filter(viewport => viewport.isTop);
    const addViewports = viewports.filter(viewport => !viewport.to.isEmpty);
...
export function getHooks(deferUntil, swapStrategy, phase, ...siblingTransitions) {
    const siblingHooks = siblingTransitions.map(sibling => getNonSiblingHooks(deferUntil, swapStrategy, phase, sibling));
...
export function getNonSiblingHooks(deferUntil, swapStrategy, phase, transitionComponents) {
    ...
    const { viewports, topViewport, addViewports, removeViewports, underlyingAdd, underlyingRemove, topFrom, topTo, } = getViewports(transitions, false /* swapStrategy.includes('parallel') && deferUntil !== 'none' */);
...
export function getInterweaved(...viewports) {
    const hooks = [];
    const viewportHooks = viewports[0] instanceof TransitionViewport
        ? viewports.filter(viewport => viewport !== void 0).map(viewport => viewport.retrieveHooks())
        : viewports;
    while (viewportHooks.length > 0) {
        for (let i = 0, ii = viewportHooks.length; i < ii; ++i) {
            ...
            if (list.length === 0) {
                ...
                hooks.push(value);
                while (value && (list === null || list === void 0 ? void 0 : list[0])) {
                    value = list.shift();
                    hooks.push(value);
                }
            }
...
function getInterweavedViewports(...viewportLists) {
    const viewports = [];
...
export function getPrepended(prefix, component, ...hooks) {
...
export function* getSingleHooks(deferUntil, swapStrategy, componentKind, phase, from, to) {
    if (from) {
        yield `${phase}.${from}.canUnload`;
    }
    if (to) {
        yield `${phase}.${to}.canLoad`;
    }
    if (from) {
        yield `${phase}.${from}.unload`;
    }
    if (to) {
        yield `${phase}.${to}.load`;
    }
            ...
            switch (componentKind) {
                case 'all-async':
                    yield* interleave((function* () {
                        if (from) {
                            yield* prepend(phase, from, ...removeHooks);
                        }
                    })(), (function* () {
                        if (to) {
                            yield* prepend(phase, to, ...addHooks);
                        }
                    })());
                    break;
                case 'all-sync':
...
export function* getParentChildHooks(deferUntil, swapStrategy, componentKind, phase, from, to) {
    ...
    const childAdd = ['canLoad', 'load', ...addHooks];
    const parentRemove = ['unload', ...removeHooks];
    const childRemove = [...removeHooks];
    if (from.c) {
        yield `${phase}.${from.c}.canUnload`;
    }
    if (from.p) {
        yield `${phase}.${from.p}.canUnload`;
    }
    if (to.p) {
    ...
    if (deferUntil === 'load-hooks' || deferUntil === 'guard-hooks') {
        if (to.c) {
            yield `${phase}.${to.c}.canLoad`;
        }
        childAdd.shift();
    }
    if (from.c) {
        yield `${phase}.${from.c}.unload`;
    }
    if (from.p) {
        yield `${phase}.${from.p}.unload`;
    }
    parentRemove.shift();
    yield `${phase}.${to.p}.load`;
    if (deferUntil === 'load-hooks') {
        if (to.c) {
            yield `${phase}.${to.c}.load`;
        }
        childAdd.shift();
    }
    switch (deferUntil) {
            ...
            switch (swapStrategy) {
                case 'parallel-remove-first':
                    ...
                    switch (componentKind) {
                        case 'all-async':
                            if (from.p) {
                                yield* interleave(from.c ? prepend(phase, from.c, ...childRemove) : prepend(phase, from.p, ...parentRemove), // getNothing(),
                                // prepend(phase, from.p, ...parentRemove),
...
function* getNothing() { }
export function assertHooks(actual, expected) {
    try {
        assert.deepStrictEqual(actual, expected);
    }
    catch (err) {
        logOutcome(filterHooks(actual), // .filter(hook => !hook.startsWith('stop.')),
...
function logOutcome(actual, expected, msg) {
    const outcome = [];
    let leftMax = 0;
...
function* prepend(prefix, component, ...calls) {
