...
import { RouterConfiguration, IRouter, RouterNavigationEndEvent } from '@aurelia/router';
import { assert, TestContext } from '@aurelia/testing';
import { CustomElement, Aurelia } from '@aurelia/runtime-html';
describe('History navigations', function () {
    var _a;
    async function createFixture(routerOptions, App, components) {
        ...
        container.register(RouterConfiguration.customize(routerOptions), ...components);
    ...
    const Foo = CustomElement.define({ name: 'foo', template: `foo`, }, (_a = class {
            load(parameters) {
    ...
    it(`show the right url and navigation flags`, async function () {
        const { tearDown, router, eventAggregator } = await createFixture({}, App, [Foo]);
        ...
        // });
        const subscription = eventAggregator.subscribe(RouterNavigationEndEvent.eventName, (event) => {
            result.url = event.navigation.path;
...
//# sourceMappingURL=history-navigations.spec.js.map