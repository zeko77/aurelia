...
describe('validation/serialization.spec.ts/validation de/serialization', function () {
    ...
    class RuleTestData {
        constructor(name, getRule, serializedRule) {
            this.name = name;
    ...
    class RulePropertyTestData {
        constructor(property, serializedProperty) {
    ...
    describe('hydrated ruleset validation works for', function () {
        class RuleHydrationTestData {
            constructor(name, displayName, ruleNameMatcher, errorMessages, target) {
                this.name = name;
                this.displayName = displayName;
                this.ruleNameMatcher = ruleNameMatcher;
...
describe('validation/serialization.spec.ts/ModelValidationExpressionHydrator', function () {
    ...
    class RuleTestData {
        constructor(name, getRule, modelRule) {
            this.name = name;
            this.getRule = getRule;
    ...
    const list = [
        ...
        ...simpleRuleList.map(({ name, getRule, modelRule }) => {
            ...
            return new RuleTestData(`${name} with tag`, function () {
                const rule = getRule();
                rule.tag = "foo";
                return rule;
            }, { [key]: { ...value, tag: "foo" } });
        ...
        ...simpleRuleList.map(({ name, getRule, modelRule }) => {
            const [key, value] = Object.entries(modelRule)[0];
            return new RuleTestData(`${name} with custom messageKey`, function () {
                const rule = getRule();
                rule.messageKey = "foo";
                return rule;
    ...
    it(`works with validationRules`, async function () {
        ...
        const validator = container.get(IValidator);
        assert.deepStrictEqual((await validator.validate(new ValidateInstruction(target))).filter((r) => !r.valid).map((r) => r.toString()), ['prop1 is required.', 'prop2 subProp1 is required.', 'prop2 subProp2 is not correctly formatted.', 'prop3 subProp1 subSubProp1 is required.']);
        assert.deepStrictEqual((await validator.validate(new ValidateInstruction(target, undefined, undefined, tag))).filter((r) => !r.valid).map((r) => r.toString()), ['prop1 is required.', 'prop2 subProp2 must be at least 42 characters.', 'prop3 subProp2 subSubProp2 is required.'], `incorrect error messages for tag ${tag}`);
    });
    const conditionals = [
        { text: 'string', when: '$object.age > 1' },
        { text: 'function', when: (object) => object.age > 1 },
    ];
    for (const { text, when } of conditionals) {
        it(`works for conditional rule - ${text}`, async function () {
            ...
            assert.deepStrictEqual((await validator.validate(instruction)).filter((r) => !r.valid).map((r) => r.toString()), [], 'error1');
            person.age = 2;
            assert.deepStrictEqual((await validator.validate(instruction)).filter((r) => !r.valid).map((r) => r.toString()), ['Name is required.'], 'error2');
            validationRules.off(Person);
        });
    }
});
