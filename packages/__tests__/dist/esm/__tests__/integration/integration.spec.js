...
describe('app', function () {
    eachCartesianJoin([
        ...
        $it(`has some textual inputs with different binding modes - ${method} - ${componentMode}`, function ({ host }) {
            ...
            const fromView = host.querySelector('#input-from-view input');
            const blurredInputTw = host.querySelector('#blurred-input-two-way input');
            const blurredInputFv = host.querySelector('#blurred-input-from-view input');
        ...
        $it(`uses a user preference control that 'computes' the full name of the user correctly - static - ${method} - ${componentMode}`, function ({ host, ctx, callCollection: { calls } }) {
            ...
            const dirty = dirtyChecker['tracked'].filter(prop => Object.is(user, prop.obj) && ['fullNameStatic', 'fullNameNonStatic', 'fullNameWrongStatic'].includes(prop.key));
        ...
        $it(`uses a user preference control that 'computes' the organization of the user correctly ${method} - ${componentMode}`, function ({ host, ctx, callCollection: { calls } }) {
            ...
            const dirtyChecker = ctx.container.get(IDirtyChecker);
            const dirty = dirtyChecker['tracked'].filter(prop => Object.is(user, prop.obj) && ['roleNonVolatile', 'locationVolatile'].includes(prop.key));
            assert.equal(dirty.length, 0, 'dirty checker should not have been applied');
            let index = calls.length;
            user.$role = 'Role2';
        ...
        $it(`uses a user preference control gets dirty checked for non-configurable property - ${method} - ${componentMode}`, function ({ host, ctx: { container } }) {
            ...
            const dirtyCheckProperty = dirtyChecker['tracked'].find(prop => Object.is(user.arr, prop.obj) && prop.key === 'indeterminate');
        ...
        [
            ...
            const options = toArray(select.querySelectorAll('option'));
        ...
        ].map(({ useCSSModule, selectedHeaderColor, selectedDetailsColor }) => $it(`uses cards to display topic details which marks the selected topic with a specific color - useCSSModule:${useCSSModule} - ${method} - ${componentMode}`, async function ({ host, ctx }) {
        ...
        $it(`cards uses inline styles - ${method} - ${componentMode}`, async function ({ host, ctx }) {
            ...
            for (const id of ['simple-style', 'inline-bound-style', 'bound-style-obj', 'bound-style-array', 'bound-style-str']) {
                assert.html.computedStyle(cardsEl.querySelector(`p#${id}`), { backgroundColor: 'rgb(255, 0, 0)', fontWeight: '700' }, `style ${id}`);
            }
