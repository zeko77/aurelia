...
function getOwnNonIndexProperties(val, showHidden) {
    if (showHidden) {
        return getOwnPropertyNames(val).filter(k => !kernel.isArrayIndex(k));
    }
    else {
        return Object_keys(val).filter(k => !kernel.isArrayIndex(k));
    }
}
...
class TestContext {
    ...
    get container() {
        if (this._container === void 0) {
            this._container = kernel.DI.createContainer();
            runtimeHtml.StandardConfiguration.register(this._container);
            this._container.register(kernel.Registration.instance(TestContext, this));
            if (this._container.has(runtimeHtml.IPlatform, true) === false) {
    ...
    get platform() {
        if (this._platform === void 0) {
            this._platform = this.container.get(runtimeHtml.IPlatform);
        }
...
function setPlatform(p) {
    exports.PLATFORM = p;
    exports.PLATFORMRegistration = kernel.Registration.instance(runtimeHtml.IPlatform, p);
}
...
function isCustomElementType(actual, message) {
    if (!runtimeHtml.CustomElement.isType(actual)) {
        innerFail({
            actual: false,
            expected: true,
            message,
...
function isCustomAttributeType(actual, message) {
    if (!runtimeHtml.CustomAttribute.isType(actual)) {
        innerFail({
            actual: false,
            expected: true,
            message,
...
function getNode(elementOrSelector, root = exports.PLATFORM.document) {
...
function matchStyle(element, expectedStyles) {
    const styles = exports.PLATFORM.window.getComputedStyle(element);
        ...
        for (const attr in attrs) {
            ...
            if (attr === 'class' || attr === 'className' || attr === 'cls') {
                ...
                if (attr.startsWith('o') && attr[1] === 'n' && !attr.endsWith('$')) {
                    ...
                    if (parts.length === 1) {
                        el.setAttribute(kernel.kebabCase(attr), value);
                    }
                    else {
    ...
    for (const child of children) {
        ...
        if (Array.isArray(child)) {
            for (const child_child of child) {
                appender.appendChild(child_child instanceof exports.PLATFORM.Node ? child_child : doc.createTextNode(`${child_child}`));
            }
        }
        else {
            appender.appendChild(child instanceof exports.PLATFORM.Node ? child : doc.createTextNode(`${child}`));
        }
    }
    return el;
};
...
function createFixture(template, $class, registrations = [], autoStart = true, ctx = TestContext.create()) {
    ...
    const App = runtimeHtml.CustomElement.define({ name: 'app', template }, $class || class {
    });
...
function recordCalls(ctor, calls) {
    ...
    for (const key in properties) {
        ...
        if (key !== 'constructor'
            ...
            if (get) {
                newGet = function () {
                    calls.addCall(this, `get ${key}`, kernel.emptyArray);
                    return Reflect_apply(get, this, kernel.emptyArray);
                };
...
exports.globalAttributeNames = globalAttributeNames;
exports.h = h;
exports.hJsx = hJsx;
