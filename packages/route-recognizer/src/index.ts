import { URLPattern } from './url-pattern-polyfill/url-pattern.js';
...
export class ConfigurableRoute<T> implements IConfigurableRoute<T> {
  private readonly _pattern: URLPattern;
  public constructor(
    public readonly path: string,
    public readonly caseSensitive: boolean,
    public handler: T,
  ) {
    this._pattern = new URLPattern({ pathname: path, caseSensitivePath: caseSensitive });
  }

  /**
   * @returns Captured parameter groups, when recognized, else `null`.
   * @internal
   */
  public _recognize(pathname: string): Record<string, string> | null {
    const result = this._pattern.exec({ pathname });
